...
# Part of the quarantined pre-chart Kubernetes generation. It renders the
# ops/any-bot-k8s workspace, which runs oshal-api-server:latest (an image nothing
# in this repo builds). The current Kubernetes path is the Helm chart at
...
if [ "${OSHAL_ALLOW_LEGACY_K8S:-}" != "1" ]; then
  printf '%s\n' \
    "REFUSED: scripts/setup-any-bot-k8s.sh is part of the quarantined legacy Kubernetes generation." \
    "It renders ops/any-bot-k8s (image oshal-api-server:latest, which nothing in this repo builds)." \
    "" \
