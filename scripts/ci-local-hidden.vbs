...
' 4 | maintainer@emeraldcoastsystemsgroup.com   | Pass --publish-image. The publisher exists since #594 and is fail-closed (only an all-green run pushes), but the nightly never asked for it, so nothing had ever published this trunk's image and the default install kept pulling the July build. The credential is loaded from .env by the publisher itself.
' =============================================================================
Set sh = CreateObject("WScript.Shell")
Set fso = CreateObject("Scripting.FileSystemObject")
repo = Replace(fso.GetParentFolderName(fso.GetParentFolderName(WScript.ScriptFullName)), "\", "/")
command = """C:\Program Files\Git\bin\bash.exe"" """ & repo & "/scripts/ci-local.sh"" --scheduled --publish-image"
