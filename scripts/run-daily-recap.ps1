<#
  run-daily-recap.ps1 — the daily driver (validated 2026-07-02). Post-close, this:
    ...
    3. STAGE:  push deck-data.json + deck.pptx (+ one-time the operator-head.png) and the build GOAL to the node.
    4. BUILD:  launch the node's local claude agent on RECAP-BUILD-GOAL.md (3 fresh-video the operator clips +
    ...
    5. COLLECT: pull the 4 pieces, assemble on the host (assemble-recap.js -> trade-recap.mp4), render the PDF.
  ...
  Any hard failure writes a note + a desktop alert so a miss is never silent.
...
function Fail($m){ Note "FAILED: $m"; try { Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.MessageBox]::Show("Daily recap $Date failed:`n$m","OSHAL recap") | Out-Null } catch {}; exit 1 }
...
function NodePull($remote,$local){ RN @('pull', "--client=$Node", "--remote=$remote", "--local=$local") | Out-Null }
...
# 1) PREFLIGHT — OPEN a Vids tab (a prior build closes its tabs) and confirm it loads signed-in (no login redirect)
...
if ((NodeShell $pf 30000) -notmatch 'VIDS_SIGNED_IN=True') { Fail "node not ready (Chrome not up / not signed into Vids on :9222). Sign in on $Node, then re-run." }
...
if (-not $SkipData) {
  ...
  docker exec oshal-local-api node /app/scripts/oshal-trade-data.js $Date 2>&1 | Out-Null
  docker exec oshal-local-api node /app/scripts/oshal-deck-data.js  $Date 2>&1 | Out-Null
...
"'HEAD='+(Test-Path '$NodeOut\the operator-head.png')" | Set-Content $chk -Encoding ascii
if ((NodeShell $chk 20000) -notmatch 'HEAD=True') { NodePush "$OUT\the operator-head.png" "$NodeOut\the operator-head.png" }
...
foreach(`$f in @('the operator-intro.mp4','the operator-overview.mp4','the operator-close.mp4','deck-narrated.mp4')){ Remove-Item "`$o\`$f" -Force -ErrorAction SilentlyContinue }
...
foreach ($f in @('the operator-intro.mp4','the operator-overview.mp4','the operator-close.mp4','deck-narrated.mp4')) { NodePull "$NodeOut\$f" "$OUT\$f" }
foreach ($f in @('the operator-intro.mp4','the operator-overview.mp4','the operator-close.mp4','deck-narrated.mp4')) { if (-not (Test-Path "$OUT\$f") -or (Get-Item "$OUT\$f").Length -lt 300KB) { Fail "piece missing/too small: $f (check $NodeOut\build.log on $Node)" } }
...
if ($LASTEXITCODE -ne 0) {
  ...
  try { Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.MessageBox]::Show("Recap $Date emailed OK but the site publish failed - see $log","OSHAL recap") | Out-Null } catch {}
