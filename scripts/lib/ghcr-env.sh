...
# 2 | maintainer@emeraldcoastsystemsgroup.com   | A SET-but-empty name is honoured as set (refuses) instead of being back-filled from .env: the repo blanks credentials to isolate a process from live keys (ci-local.sh seq 13), and the pre-existing publisher guard relies on exactly that.
#
...
oshal_ghcr_load_one() {
  ...
  # SET wins, even when set to empty: the repo blanks credentials to isolate a process from
  # live keys, and an explicit empty value must refuse, not quietly back-fill from the file.
  [ -n "${!name+x}" ] && return 0
