 ...
 * 5 | maintainer@emeraldcoastsystemsgroup.com   | Reuse the shared version-aware connector-token codec (legacy/k2/v2 plus hkdf1 DEK wrappers); refreshed access tokens now stay in caller-owned v2 envelopes instead of being rewritten under the shared legacy key.
 */
/*
 ...
 * wrapped_dek may be a current `hkdf1:` wrapper or an unprefixed legacy wrapper.
...
async function main() {
  ...
  try {
    ...
    if (!token) {
      ...
      await client.query("UPDATE oshal_connections SET access_token=$3, expiry=$4, updated_at=NOW() WHERE provider='google' AND account_email=$1 AND user_sub=$2", [row.account_email, row.user_sub, await connectorTokenCrypto.encryptToken(client, row.user_sub, tok.access_token), tok.expires_in ? new Date(Date.now() + tok.expires_in * 1000) : null]).catch(() => {});
    }
