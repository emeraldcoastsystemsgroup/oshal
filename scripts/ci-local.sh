...
# 33 | maintainer@emeraldcoastsystemsgroup.com   | Make the mock e2e posture explicit: the noop lane declares the model-less test deployment and the stable mock subject is the operator for operator-gated browser proofs; production never inherits either setting.
...
# 2026-07-29: the scan covers `src tests scripts`, not `src` alone. The 1000-code-line cap applies
# to source, tests, and logic-bearing config (CLAUDE.md), but the max-lines rule was scoped to
# src/**/*.ts{,x}, so the three files actually over the cap — including an 1850-code-line .mjs, the
# worst file in the repo — exited 0 under `npx eslint src`. eslint.config.mjs now applies max-lines
...
gate_e2e() {
  ...
  (cd "$GATE_SRC" && \
    ...
    MOCK_OIDC=true MOCK_OIDC_ALLOW_HEADER=true CI=true FORCE_LLM_PROVIDER=noop OSHAL_NO_AI=true OSHAL_OPERATOR_SUBS=mock-user-001 RUN_MIGRATIONS=true \
...
# Publish the image this run built and scanned. Derived, never hardcoded: the owner comes from
# the repository's own origin remote, and OSHAL_CI_GHCR_IMAGE overrides the whole repo path for a
# fork or a private registry. `oshal-bot` is the sanctioned image name (CLAUDE.md) and the only
# tag the installer pulls.
gate_publish_image() {
  local remote="${OSHAL_CI_GHCR_IMAGE:-}"
  if [ -z "$remote" ]; then
    local origin owner
...
if [ "$PUBLISH_IMAGE" = "1" ]; then
  ...
  elif [ "${SCHEDULED:-0}" = "1" ] && [ "${SOURCE_POSTURE:-unresolved}" != "scheduled-origin-main" ]; then
    # A scheduled run that could not pin origin/main judged whatever the checkout had as HEAD -
    # routinely a feature branch in this multi-agent tree. Green or not, that is not the trunk,
