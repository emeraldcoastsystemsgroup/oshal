...
if ($pkgSet.Count -gt 0) {
  ...
  $storeTmp = Join-Path $env:TEMP ("oshal-store-" + [guid]::NewGuid().ToString('N').Substring(0,8))
  New-Item -ItemType Directory -Force -Path $storeTmp | Out-Null
  $tarball = Join-Path $storeTmp 'store.tar.gz'
  Invoke-WebRequest -UseBasicParsing -Uri 'https://codeload.github.com/emeraldcoastsystemsgroup/oshal-apps/tar.gz/refs/heads/main' -OutFile $tarball
  tar -xzf $tarball -C $storeTmp
  $storeDir = Get-ChildItem $storeTmp -Directory -Filter 'oshal-apps-*' | Select-Object -First 1
  foreach ($p in $pkgSet) {
    $src = Join-Path $storeDir.FullName $p
    if (-not (Test-Path $src)) { Note "SKIP $p - not in the public store"; continue }
    docker run --rm -v oshal-local_oshal_workspace:/ws alpine:3 sh -c "[ -d /ws/deployed-apps/$p ]" *> $null
    if ($LASTEXITCODE -eq 0) { Note "SKIP $p - already installed (never register a bot/surface twice)"; continue }
    $srcU = $src -replace '\\','/'
    docker run --rm -v oshal-local_oshal_workspace:/ws -v "${srcU}:/src:ro" alpine:3 sh -c "mkdir -p /ws/deployed-apps && cp -r /src /ws/deployed-apps/$p"
    Note "staged $p"
  }
  Remove-Item -Recurse -Force $storeTmp
}
