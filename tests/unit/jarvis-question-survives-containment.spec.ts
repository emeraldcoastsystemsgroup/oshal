...
describe('the user question survives the bot node prompt containment', () => {
  it('keeps the question when the context blocks are larger than the whole window', () => {
    ...
    const assembled = assembleJarvisBotMessage(HUGE_CONTEXT, QUESTION, QUESTION);
  ...
  it('returns the user part unchanged when there is no context block', () => {
    expect(assembleJarvisBotMessage('', QUESTION, QUESTION)).toBe(QUESTION);
  });

  it('repeats the question as the closing line so it is also the most recent thing said', () => {
    const assembled = assembleJarvisBotMessage(HUGE_CONTEXT, QUESTION, QUESTION);
    expect(assembled.trimEnd().endsWith(QUESTION)).toBe(true);
  });
});

describe('typed access tools spend their operations/targets JSON only on an access ask', () => {
