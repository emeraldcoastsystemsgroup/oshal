 ...
 * 2 | maintainer@emeraldcoastsystemsgroup.com | The bare-DDL allowlist is EMPTY. The three bootstraps it named - trading-bar-store, trading-config-overrides and trading-strategy-lab-store - now take the family lock, so the case asserts that no trading module issues unlocked DDL at all rather than that the unlocked ones are the ones we already knew about.
 */
 ...
 *  take no advisory lock.
 *
 *  EMPTY as of 2026-09-19, which makes the case below strictly stronger than the allowlist it
 *  replaces: it now asserts that NO trading module issues unlocked DDL at all, rather than that
 *  the unlocked ones are the three we already knew about. The three that were here -
 *  trading-bar-store, trading-config-overrides and trading-strategy-lab-store - now route through
 *  runRuntimeSchemaBootstrap with the family key. Re-adding a name here needs its own reason. */
const BARE_DDL_BOOTSTRAPS: string[] = [];

/** The helper must be CALLED, not merely imported. Measured 2026-09-19: removing the call while
 *  leaving the import passed every case, because the check was a substring over the whole file and
 *  `noUnusedLocals` is not enabled, so nothing else would have objected either. */
const CALLS_HELPER = /runRuntimeSchemaBootstrap\s*\(/;
...
describe('every trading schema bootstrap takes the family advisory lock', () => {
  ...
  it('covers the whole family, not a sample', () => {
    ...
    expect(locked.length).toBe(modules.filter(m => CALLS_HELPER.test(m.source)).length);
