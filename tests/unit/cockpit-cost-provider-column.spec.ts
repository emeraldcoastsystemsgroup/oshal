 ...
 * 6 | maintainer@emeraldcoastsystemsgroup.com   | Corrects "first-wins" to LAST-wins in the title, the comment and entry 4. The expression was `providerId || existingAgentSummary.providerId || ''` and providerId is always truthy, so the row being folded IN always won, not the one already there. The fix is the same either way - the description was not.
 */
...
describe('the cockpit Cost-by-Bot "Provider" column is fed, not structurally dead', () => {
  ...
  it('the direct cost summary widens across providers too, instead of keeping the last one seen', () => {
    // This path is preferred over the task rollup (cockpit-ticket-activity-route), so a
    // last-wins provider here would render one confident provider and one confident ADR-127
