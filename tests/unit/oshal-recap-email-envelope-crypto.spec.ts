 ...
 * -----------------------------------------------------------------------------
 * 1 | maintainer@emeraldcoastsystemsgroup.com   | Regression guard for the envelope-crypto drift hitting scripts/oshal-recap-email.js — the same v2 per-user-DEK drift already guarded for oshal-gmail.js (oshal-gmail-envelope-crypto.spec.ts). The daily recap email died with "Unsupported state or unable to authenticate data" the moment the operator's Google token refreshed into a `v2:` blob, which silently stopped the recap publish chain (email Fail() exits before archive + site publish). Pins that the CLI's exported decryptToken reads BOTH a v2 per-user-DEK blob and a legacy KEK blob.
 */
...
process.env.SESSION_SECRET = 'test-session-secret-recap-envelope-guard';

const recapEmail = require('../../scripts/oshal-recap-email.js') as {
  key: () => Buffer;
...
describe('oshal-recap-email.js connector-token decrypt is envelope-aware (v2 + legacy)', () => {
  const KEK = recapEmail.key(); // SHA256(SESSION_SECRET) — same derivation the controller uses
  ...
  it('decrypts a v2 per-user-DEK envelope blob (the exact case that broke the recap email)', async () => {
    ...
    const out = await recapEmail.decryptToken(fakePool(wrappedDek), 'sub-v2-happy', v2blob);
  ...
  it('still decrypts a legacy unprefixed single-KEK blob', async () => {
    ...
    const out = await recapEmail.decryptToken(fakePool(null), 'sub-legacy', legacy);
    expect(out).toBe(token);
  });
  ...
  it('throws a clear error when a v2 blob has no DEK row', async () => {
    ...
    await expect(recapEmail.decryptToken(fakePool(null), 'sub-missing-dek', v2blob)).rejects.toThrow(/DEK/);
  });
});
