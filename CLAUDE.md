...
- **Workflow Studio authoring is design-time; Publish now compiles to runtime.** `/workflow-studio/` authors and validates workflow graphs. The **Publish** action is live: it compiles a definition to a caller-scoped manifest and loads it as a real ticket queue (single-shot → a one-stage nodeGraph, multi-stage → a compiled nodeGraph whose `approvalAfter` stages become executable `approval-gate` nodes, and a full branching/parallel canvas → the same nodeGraph shape — all three on the process-definition engine, shipped 2026-07-05). **There is no `staged` executor.** It was retired in favour of the graph engine (`dispatch-routing.ts`), so a hand-authored manifest declaring `pipeline: staged` routes to `manifest-worker` and runs only `workerBot`, dropping every approval gate silently — see [backlog/clean-kernel-repair.md](docs/backlog/clean-kernel-repair.md) CKR-10, alongside the workflows registered from `swarm-apps/*.yaml` via `WorkflowPipelineRegistry`. A natural-language **talk-to-build** assist drafts the canvas graph; only agentic authoring that composes **brand-new** agents/goals from a prompt is still roadmap. Don't treat the studio as a second orchestration engine: the runtime (queue-manager + registry) stays the authority; Publish emits into it, it does not replace it.
