 ...
 * -----------------------------------------------------------------------------
 * 4 | maintainer@emeraldcoastsystemsgroup.com   | Entry 3 was wrong and entry 2 was right: the agent_config record is NOT the per-bot switch row — it is machinery-written (manifest seeding, broadcast-up, config push; 70 such rows on the operator box, none a person's) and it keeps exactly the rank entry 2 measured for it, below a non-cline registry harness, so that a fleet-default write reaches every bot. The switch rows (bot's own row, written by an operator through PUT /runtime; the fleet default) are the only tier-0 records, and the installed snapshot no longer reflects agent_config. A save through PUT /runtime writes BOTH the ADR-034 record (push-before-persist, unchanged) and the bot's own switch row, so the notes say that.
...
export type BotProviderSource =
  ...
  | 'registry-harness'
  /** The per-agent DB record (agents.api_provider_id / agent_config providerId) — machinery-written, never a switch. */
  | 'agent-profile'
...
export function resolveEffectiveBotProvider(inputs: BotProviderInputs): EffectiveBotProvider {
  ...
  if (harness && harness !== PASS_THROUGH_HARNESS) {
    return {
      ...
      precedenceNote: `The bot registry declares harness '${harness}' for this bot and no switch row `
        + "overrides it. Saving a provider here writes the bot's own switch row (and its ADR-034 runtime "
        + 'record), resolved ABOVE the registry literal; the fleet default (one row) sits between the two.',
    };
  }
  ...
  if (dbProvider) {
    return {
      ...
      providerOverridable: true,
      modelOverridable: true,
      precedenceNote: 'This bot runs on its saved runtime record. The registry pins no harness for it and '
        + 'no switch row exists, so the record is the highest tier that applies.',
    };
  }
  ...
  return {
    ...
    precedenceNote: `${rung} decides: '${resolution.providerId}' runs through harness '${resolution.harnessType}'. `
      + (resolution.source === 'bot-row'
        ...
        : 'No per-bot switch row exists, so the fleet default outranks the registry literal and the runtime record alike; save a provider here to give this bot its own row.'),
  };
}
