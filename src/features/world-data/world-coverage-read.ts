...
/** Bounds archive access to eight subject-index scans; never sorts the entire article archive. */
export async function readWorldCoverage(pool: Pool, at = new Date()) {
  ...
  const results = await Promise.allSettled([
    ...
    pool.query(timed(`SELECT s.entity, s.label, a.title, a.description, a.link, a.outlet,
      ...
      (SELECT entity, label FROM world_subjects WHERE last_seen <= $1 AND last_seen > $1::timestamptz - interval '24 hours'
       ORDER BY last_seen DESC, entity LIMIT 8) s
      CROSS JOIN LATERAL (SELECT title, description, link, outlet, first_seen_at, pub_date
        FROM world_items WHERE entity_id = s.entity AND first_seen_at <= $1
          AND first_seen_at > $1::timestamptz - interval '24 hours'
        ORDER BY first_seen_at DESC, item_hash LIMIT 3) a
      ORDER BY a.first_seen_at DESC, s.entity LIMIT 24`)),
