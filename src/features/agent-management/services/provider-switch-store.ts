 ...
 * 4 | maintainer@emeraldcoastsystemsgroup.com   | Carries fallback_order (migration 148): the ordered provider chain an administrator writes. Passing undefined leaves an existing chain untouched, so changing a provider does not silently wipe the failover order; an explicit array replaces it, and an explicit [] means "no failover, fail visibly" rather than "inherit".
 */
...
interface SwitchRowRecord {
  ...
  /** Postgres TEXT[]; null when the row inherits the next precedence rung. */
...
function toRow(record: SwitchRowRecord): ProviderSwitchRow {
  ...
  return {
    ...
    // null and [] are different answers: inherit the next rung vs no failover, deliberately.
    fallbackOrder: Array.isArray(record.fallback_order) ? record.fallback_order : null,
...
export class ProviderSwitchStore {
  ...
  async listAll(): Promise<ProviderSwitchRow[]> {
    const result = await this.pool.query<SwitchRowRecord>(
      `SELECT scope_id, provider_id, model_id, fallback_order, updated_by, updated_at
  ...
  ): Promise<ProviderSwitchRow> {
    // `undefined` leaves an existing chain alone (a provider change must not silently wipe the
    // administrator's failover order); an explicit array — including [] — replaces it.
    const writesChain = fallbackOrder !== undefined;
    const result = await this.pool.query<SwitchRowRecord>(
      `INSERT INTO oshal_bot_provider_switch (scope_id, provider_id, model_id, fallback_order, updated_by, updated_at)
       VALUES ($1, $2, $3, $4, $5, NOW())
       ON CONFLICT (scope_id) DO UPDATE SET
         provider_id = EXCLUDED.provider_id,
         model_id = EXCLUDED.model_id,
         fallback_order = CASE WHEN $6::boolean THEN EXCLUDED.fallback_order
                               ELSE oshal_bot_provider_switch.fallback_order END,
         updated_by = EXCLUDED.updated_by,
         updated_at = NOW()
       RETURNING scope_id, provider_id, model_id, fallback_order, updated_by, updated_at`,
      [scopeId, providerId, modelId, writesChain ? (fallbackOrder ?? null) : null, updatedBy, writesChain],
    );
    const row = toRow(result.rows[0]);
    logger.info({ scopeId, providerId, modelId, fallbackOrder: row.fallbackOrder, updatedBy }, 'Provider switch row written');
    return row;
  }
