...
import { gucEnabled, ownPoolConnectionErrors, wrapPoolWithGuc } from '@/shared/services/database';
...
function getPool(): Pool {
  if (!sharedPool) {
    const raw = ownPoolConnectionErrors(new Pool({
      connectionString: process.env.DATABASE_URL,
      max: resolvePoolMax(process.env.RAG_DB_POOL_MAX, 4),
      application_name: postgresApplicationName(process.env.RAG_PGAPPNAME, 'oshal-api-rag'),
    }), 'rag');
