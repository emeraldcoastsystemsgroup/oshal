 ...
 * 2 | maintainer@emeraldcoastsystemsgroup.com   | Exports AntigravityCliHarnessAdapter beside the Gemini one: two registered Google terminal agents, both selectable api-side. Neither is usable as a fallback rung - both carry botNodeRuntime: null, so no bot-node runtime resolves them.
 * 3 | maintainer@emeraldcoastsystemsgroup.com   | Follow-on correction: Antigravity now has a dedicated bot-node runtime; Gemini CLI remains api-side-only.
 */
