...
/** @description Fill absent runtime keys under the database conflict lock while preserving explicit blank, null and disabled choices.
 * @param pool - Application database pool.
 * @param agentId - Exact bot receiving initial runtime defaults.
 * @param seed - Proven provider/model defaults, not an operator override.
 * @returns Whether a usable authoritative provider remains after the atomic merge.
 */
...
/** @description Persist each owned bot before activation can expose package dispatch.
 * @param pool - Application database pool.
 * @param manifest - Validated owning application manifest.
 * @param manifestPath - Owning package location for persona resolution.
 * @param defaults - Deployment runtime resolver; omitted only by legacy callers.
 * @returns Completion after all authoritative bot records are initialized.
 */
