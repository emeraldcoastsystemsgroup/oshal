...
export const FUTURES_RESEARCH_SCENARIOS: Scenario[] = [{
  ...
  steps: [{ id: 'isolated', app: 'intelligent-trades', label: 'Isolated research regression runner', run: async () => ({
    ...
    detail: 'No tests ran from this browser step. Run npx vitest run --no-file-parallelism futures-research- futures-prediction- futures-review-forward- from the framework checkout with Docker available. Follow docs/apps/trading/futures-research-loop.md for console acceptance; no live provider or nightly completion is claimed.',
  }) }],
}];
