...
interface AppLoader {
  /** The scope is REQUIRED in shape even though the parameter is optional: omitting it lets the
   *  install-owner adoption stamp a row this caller owns. Callers with an identity pass it. */
...
export function createSwarmPackRoutes(appLoader?: AppLoader): Router {
  ...
  router.post('/:name/deploy', async (req: Request, res: Response) => {
    ...
    try {
      ...
      // The caller's sub is passed EXPLICITLY. Without it the scope arrives empty and
      // withInstallOwner stamps OSHAL_INSTALL_OWNER_SUB — which every new install now sets — so
      // one user's deployed pack was owned by the install owner and the install owner was made
      // its administrator. Naming the caller is what keeps adoption to rows that genuinely have
      // nobody: adoption only fires when ownerSub is absent.
      const callerSub = (req as unknown as { oidc?: { user?: { sub?: string } } }).oidc?.user?.sub;
      await appLoader.loadApp(manifestPath, callerSub ? { ownerSub: String(callerSub) } : undefined);
