 ...
 * 5 | maintainer@emeraldcoastsystemsgroup.com   | ADR-085 Wave 3 carve: the Email Summarizer app surface (createEmailRoutes + inbox/my-day/social pages, per-user digest storage, bot-run summary/draft, and its 428-gated send route) moved to the email-summarizer store package. This module retains the shared senders and metadata helpers; the path stays stable because notification, Jarvis, Career Hunter, Presentations, and the store package import it here. The header-injection fence remains guarded in tests/unit/risky-write-guards.spec.ts.
 *
 * @module email-routes
 */
