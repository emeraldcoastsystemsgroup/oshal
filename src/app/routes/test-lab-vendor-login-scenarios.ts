...
export const VENDOR_LOGIN_SCENARIOS: Scenario[] = [{
  ...
  description:
    'The operator signs into Codex, Claude and Google in his own browser, on his own machine, and the oshal client pushes each vendor CLI\'s login file to its import route. This scenario exercises only the closed door: an anonymous caller reaching import, sign-out or the connect-state probe on any of the three mounts. The open door needs a real browser login and is not something the Lab may perform.',
  regressionTests: [
    ...
    // Where a Gemini turn RUNS once a login has been pushed — the CLI harness, not the HTTP lane.
