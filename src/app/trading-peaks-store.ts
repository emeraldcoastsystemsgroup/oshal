 ...
 * 4 | maintainer@emeraldcoastsystemsgroup.com   | ensurePeaksTable now goes through runRuntimeSchemaBootstrap under the SCHEMA_LOCK_KEYS.trading advisory lock instead of issuing eleven bare pool.query DDL statements. It is the only trading bootstrap with NO memo - the comment above says it is "called before each use" - so it was the family's most frequent racer: two callers interleaving its `DROP TRIGGER IF EXISTS` / `CREATE TRIGGER` pair raise 42710, and the check-then-`CREATE POLICY` pair does the same. Going through the shared bootstrap also means the table/columns are asserted afterwards on a non-owner role rather than the first owner-only statement aborting the rest.
 *
 ...
 * Serialised on the trading advisory lock: this bootstrap is deliberately NOT memoized, so two
 * concurrent fires would otherwise interleave the DROP/CREATE TRIGGER pair and the check-then-
 * CREATE POLICY pair and one of them would fail with 42710.
...
export async function ensurePeaksTable(pool: AppContext['pool']): Promise<void> {
  ...
  await runRuntimeSchemaBootstrap({
    pool, moduleName: 'trading peaks', lockKey: SCHEMA_LOCK_KEYS.trading,
    statements: [
      ...
      ...buildOwnerRlsPolicyStatements('oshal_trading_peaks', 'user_sub'),
    ...
    requirements: [{
      table: 'oshal_trading_peaks',
      columns: ['user_sub', 'mode', 'symbol', 'peak_price', 'updated_at', 'book_id'],
    }],
  });
}
