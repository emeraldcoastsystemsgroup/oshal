 ...
 * 17 | maintainer@emeraldcoastsystemsgroup.com   | Registered the 'antigravity-cli' harness (Google's `agy`) beside gemini-cli: its own model/binary env vars (ANTIGRAVITY_MODEL, ANTIGRAVITY_CLI_PATH) falling through to the shared Google ones, because both CLIs authenticate with GEMINI_API_KEY but do not necessarily run the same model id. Both remain selectable api-side. Neither can be a fallback RUNG: HARNESS_BY_ID gives both botNodeRuntime: null and neither spelling is a ProviderRegistry id, so resolveBotNodeSwitch returns null and the node builds no rung for either.
 * 18 | maintainer@emeraldcoastsystemsgroup.com   | Follow-on correction: antigravity-cli now has its own bot-node runtime and can be a node rung; gemini-cli remains api-side-only. API-side factory behavior here is unchanged.
 */
