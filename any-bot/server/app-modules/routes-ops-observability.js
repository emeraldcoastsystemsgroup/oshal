...
function registerOpsObservabilityRoutes(application) {
    ...
    application.app.get('/api/qm/activity', async (req, res) => {
      ...
      try {
        ...
        const agents = {};
        for (const key of agentKeys) {
          const data = await redis.get(key); // FIX: AgentRegistry uses STRING, not HASH
          const ttl = await redis.ttl(key);
          if (data) {
            try {
              const parsed = JSON.parse(data);
              const agentId = parsed.agent_id || key.replace('queue-manager:agent:', '');
              agents[agentId] = {
                ...parsed,
                ttl,
              ...
              logger.warn(`Failed to parse agent data for ${key}: ${e.message}`);
            }
          }
        }
        ...
        for (const key of phaseKeys) {
          ...
          if (data) {
            try {
              const parsed = JSON.parse(data);
              phases.push({ ticketId: key.replace('qm:ticket_phase:', ''), ...parsed, ttl });
            } catch(e) { phases.push({ ticketId: key.replace('qm:ticket_phase:', ''), raw: data, ttl }); }
          }
        }
        ...
        for (const key of dispatchKeys) {
          ...
          const ttl = await redis.ttl(key);
          if (data) {
            try { dispatches.push({ ticketId: key.replace('qm:dispatching:', ''), ...JSON.parse(data), ttl }); }
            catch(e) { dispatches.push({ ticketId: key.replace('qm:dispatching:', ''), raw: data, ttl }); }
          }
        }

        // 4. Task-Ticket Mappings
        const mappingKeys = await redis.keys('qm:ticket_task:*');

        // 5. Cooldown/Lock keys
        const cooldownKeys = await redis.keys('qm:cooldown:*');
        const lockKeys = await redis.keys('qm:lock:*');
        ...
        res.json({
          ...
          agentTicketMappings: {}, // TODO: implement if needed
          cooldowns: {},
          locks: {},
        });
      } catch (err) {
...
module.exports = { registerOpsObservabilityRoutes };
