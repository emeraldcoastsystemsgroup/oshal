...
| Item | Today | Target | Tracked in |
|---|---|---|---|
| **Artifact exchange — the source direction** | The registry already parses, validates and stores each app's `provides:` declarations, and the handle rail that would carry the result is shipped and proven in the "Send to…" direction. Nothing consumes `provides:` yet, so every app still hand-builds its own file picker. | The inverse gesture: one shared "pick an artifact" component that lists sources across apps and hands back a handle, replacing bespoke pickers, plus an NL leg so "Jarvis, send this to X" resolves against the same catalog the menu uses | [ADR-139](docs/adr/139-artifact-exchange-send-to-registry.md), [BACKLOG](docs/BACKLOG.md) |
