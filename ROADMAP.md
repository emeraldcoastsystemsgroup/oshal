...
|---|---|---|---|
| **Artifact exchange — "Send to…"** | **Shipped** — any artifact surface tags an object with three `data-artifact-*` attributes and one script include; the menu offers 12 registered destinations (email compose, OSHAL Storage, RAG ingest, Jarvis summarize, Portrait Studio, Career, AI Office, Tutor, Print inbox, D&D, Kid Lens, Spaces), dispatching over owner-bound short-TTL handles that carry a locator, never bytes | Sources with no byte-serving URL still cannot participate (the task-explorer Files tab, RAG Center documents, and every client-generated export such as a composed social image or a payroll file) — they wait on a mint-with-bytes handle variant, a decision the ADR has to record either way | [ADR-139](docs/adr/139-artifact-exchange-send-to-registry.md), [BACKLOG](docs/BACKLOG.md) |
