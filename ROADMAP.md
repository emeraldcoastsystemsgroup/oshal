...
| Item | Today | Target | Tracked in |
|---|---|---|---|
| **Artifact exchange — the source direction** | Shared picker implemented: registered app sources plus connected storage, owner-bound handles, Portrait Studio adoption with the old modal removed; local HTTP/file/browser acceptance passes. | Protected core merge and deployed signed-in acceptance; the natural-language handoff remains a separate Stage 4b item. | [ADR-139](docs/adr/139-artifact-exchange-send-to-registry.md), [BACKLOG](docs/BACKLOG.md) |
