...
|---|---|---|---|
| **Artifact exchange — "Send to…"** | **Shipped** — any artifact surface tags an object with three `data-artifact-*` attributes and one script include; the menu offers 12 registered destinations (email compose, OSHAL Storage, RAG ingest, Jarvis summarize, Portrait Studio, Career, AI Office, Tutor, Print inbox, D&D, Kid Lens, Spaces), dispatching over owner-bound short-TTL handles. A handle normally carries a **locator** and the redeem re-fetches it server-side as the minting caller; since Amendment D (2026-09-09) a second mint **carries the bytes**, so a source with no byte-serving URL to point at can participate too — same owner binding and TTL, memory only, per-handle and per-sub byte caps, and no change in any destination. The task-explorer Files tab, whose route answers a JSON preview envelope rather than a file, is the live proof | The other client-generated exports (Switchboard's composed image, camera captures, payroll files, ocean-lab CAD, workflow-studio JSON) have the rail now but are not tagged yet — ordinary rollout, one tag each. RAG Center documents stay out: a retrieved corpus chunk is not a file, so what a handle would carry there is an open product question rather than a plumbing gap | [ADR-139](docs/adr/139-artifact-exchange-send-to-registry.md), [BACKLOG](docs/BACKLOG.md) |
| **Antigravity CLI harness** | **Registered, not runnable** — `antigravity-cli` is in the typed harness inventory and `HARNESS_FACTORIES`, selectable api-side like any other provider (it is **not** usable as a fallback rung: `HARNESS_BY_ID` gives it `botNodeRuntime: null`, so no bot-node runtime resolves it), with the published headless contract (`agy -p … --output-format json --model … --print-timeout …`), the single-object JSON envelope where `status` outranks the exit code, and the provider settings file the CLI needs beside `GEMINI_API_KEY`. It refuses on this image with a measured cause: Google publishes no musl build (manifest 404) and the glibc binary does not relocate under `gcompat`. `gemini-cli` runs today on the same key. | A glibc base for the bot image — proven not to regress the musl-dependent cline binary or reintroduce the native-addon segfault class — plus the node-sizing decision: every bot container carries ~1.08 GB of CLI harnesses it may never invoke, which is paid per node on a one-container-per-bot topology. Also unestablished: whether `agy`'s SSH device-code account login rides an entitlement rather than the API key quota. | [backlog/antigravity-and-node-footprint.md](docs/backlog/antigravity-and-node-footprint.md) |
| **Workflow Studio compile-to-runtime** | **Shipped** — the canvas **Publish** action compiles a definition to a caller-scoped manifest and loads it live as a ticket queue (single-shot → a one-stage nodeGraph, multi-stage → a compiled nodeGraph whose `approvalAfter` stages become executable `approval-gate` nodes, and a full branching/parallel canvas → the same nodeGraph shape — all three on the process-definition engine, 2026-07-05). There is no `staged` executor; the interim one was retired in favour of the graph engine. | Natural-language authoring that composes brand-new agents/goals (not just the graph over existing bots) — tracked under **Agentic Workflow Studio** below. | [BACKLOG.md](docs/BACKLOG.md), [docs/architecture/workflow-studio-framework.md](docs/architecture/workflow-studio-framework.md) |
...
| Item | Today | Target | Tracked in |
|---|---|---|---|
| **Biometric privileged-access module** | Not built. Moved here from the backlog on 2026-09-15 by operator decision: wanted eventually, not being built now. | Pluggable face and/or voice enrollment whose signed challenge result can satisfy a high-privilege endpoint, with a non-biometric fallback and a documented safe path for devices without a camera or microphone. | Operator decision, 2026-09-15 |
| **Alexa-exclusive device control** | Not built, and deliberately not wired — `connectors-routes.ts` records "Alexa is intentionally NOT wired" and no `alexa` provider exists in the connector registry. Devices are reached through SmartThings, Matter or Home Assistant instead, which needs no certification. | A certified Amazon Smart Home Skill, which is the only way a third party controls an Alexa-exclusive device: Login with Amazon registered under the business account plus Amazon certification review measured in weeks to months. **Operator decision 2026-09-22: take the cheap path now and defer the paid one — the option to pay stays open and is revisited only if a device appears that nothing else can control.** | [ROADMAP](ROADMAP.md), reference staging in [partner-app-registration.md](docs/partner-app-registration.md) |
| **Artifact exchange — the source direction** | Shared picker implemented: registered app sources plus connected storage, owner-bound handles, Portrait Studio adoption with the old modal removed; local HTTP/file/browser acceptance passes. | YAML-guided Jarvis handoff also implemented with owner-bound selection and existing dispatch gates; protected core merge and deployed signed-in/model acceptance remain. | [ADR-139](docs/adr/139-artifact-exchange-send-to-registry.md), [BACKLOG](docs/BACKLOG.md) |
...
| **Self-hosted local-LLM swarm profile** | Ollama/LM Studio/LiteLLM are wired; `gpt-oss-120b` is offered via the Cerebras API. Historically, `gpt-oss-20b` ran in the ai-lab Kubernetes configuration on AWS as a single-threaded POC swarm; `gpt-oss-120b` was not run there because the AWS spend (~$100k/mo) was not justified for a POC. **Hardware decision, operator 2026-09-21: deferred, and moved here off the backlog.** The binding constraint is not the GPU budget: the hardware available supports no more than a ~7k-token context window, and the agent harness this platform runs cannot operate in a window that small. The controller box has an integrated Intel Arc 140V (~8 GB shared) and no discrete GPU; `OLLAMA_HOST` is unset and the `local-llm` compose profile is off. No tier was selected and no purchase is implied. | A documented, repeatable local-LLM swarm profile (including `gpt-oss` on Ollama) with benchmarks. | [ROADMAP](ROADMAP.md) — revisit when hardware can serve a context window the harness can use |
...
| **Simulator comparison — PteroSim** | Our simulators are purpose-built and headless. `aero-lab` searches a persistent-flight design space (AeroSandbox/NeuralFoil/casadi in an engine container, on a custom two-timescale 3-DOF integrator — JSBSim was rejected there after it was measured manufacturing energy at wind-field steps, which matters for a multi-day energy budget). The `embodied` physics lab ([ADR-152](docs/adr/152-embodied-physics-and-training-lab.md)) trains against MuJoCo built from the parts model, with the kinematic sim as the certification gate. The embodied drone node flies real PX4 SITL (SIH) over MAVLink. None of them render a world, and none run a full 6-DOF flight-dynamics model against a stock flight stack, so we have no outside reference to check a flight result against. | Evaluate [PteroSim](https://pterolabs.ai/) as that reference — Unreal Engine 5 scene, JSBSim 6-DOF dynamics, PX4/ArduPilot/Betaflight SITL over the lockstep Simulator MAVLink API (TCP 4560), gRPC and a Python SDK, Windows and Ubuntu — and decide per simulator whether it is an oracle to cross-check against, a replacement, or neither. Price in the licence before depending on it: proprietary, and the free tier is one concurrent vehicle (F450 only on the PX4 path), two-hour sessions with a five-hour cooldown, and no commercial use; multi-vehicle, faster-than-real-time and commercial rights are paid, and PX4 lists the integration as community-supported. | [BACKLOG.md](docs/BACKLOG.md) |
| **Marketing suite — launch to growth, end to end** | Campaign engine shipped as the store `marketing-engine` package: campaigns with stage gates, four inline bots, per-channel owner consent with daily caps and confirm, manual publishing to LinkedIn/Mastodon/Bluesky, one confirm-gated single-recipient email send, scorecard ingest, the weekly review ticket, and a budget proposal ledger. X/Facebook publishing lives in Switchboard. | Audience with recipient consent and compliant broadcasts (double opt-in, one-click unsubscribe, suppression, bounce and complaint handling); campaign budgets held as finance projects with actuals and variance; sequences under per-sequence standing authorization; SMS with TCPA/CTIA consent and STOP/HELP; campaign-level attribution; paid-ads spend and approved budget changes; one Marketing Suite front door (ADR-141 group). | [spec](docs/apps/marketing-suite-spec.md), [market scan](docs/business/marketing-suite-market-research.md), [BACKLOG.md](docs/BACKLOG.md) |
| **DevOps cockpit Phase 2+ (Later slices)** | First slice is in backlog: a NATed node self-registers and a round trip works across the connection rail. | Later slices moved here from backlog (operator decision 2026-09-22): topology discovery from logged-in CLIs into the graph, Connect-Vault with live traffic lights, Terraform and Kubernetes context discovery/overrides, bidirectional transport selection (Headscale overlay, A2A gateway, or HTTPS long-poll/SSE), and specialist tasks running with short-TTL brokered credentials while apply/deploy remains human-gated. | [connectivity design](docs/architecture/devops-cockpit-connectivity.md), [ROADMAP](ROADMAP.md) |
| **LoRA local fine-tuning studio & character training** | Manifest, database schema, and route scaffolding exist in store package `lora`. Training requires dedicated discrete GPU acceleration (VRAM ≥ 16GB). **Hardware decision, operator 2026-09-23: iced and moved here off the backlog.** Controller box has an integrated Intel Arc 140V (~8 GB shared memory) and no discrete GPU. | Local fine-tuning and character checkpoint training studio (ComfyUI / Kohya / diffusers pipeline) with automated scorecard evaluation and gallery rendering when compatible compute/GPU hardware is available. | [ROADMAP](ROADMAP.md), `oshal-applications/lora/` |
| **Kibot / alternative Futures data streaming — discussion only** | The active Schwab connection is the operator's preferred finance data source. No Kibot ES/CL archive is owned or mounted, and Kibot must not block the paper/research Futures loop. Read-only live probing found recent dated ES/CL volume-bearing candles; owner-private forward collection has a one-shot installed receipt, but full historical depth remains unproven. | Discuss whether a separate licensed Futures history/streaming provider is ever needed after measuring Schwab's actual retention, contract-level gaps, timestamp/volume quality and permitted internal storage. Compare cost and operational burden. Any later provider purchase, installation or live trading remains a separate decision. | Operator direction, 2026-09-25; [Futures backlog](docs/BACKLOG.md) |
