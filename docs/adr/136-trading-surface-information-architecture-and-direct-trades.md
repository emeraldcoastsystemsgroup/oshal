...
- The earnings blackout gate (`TRADING_EARNINGS_GATE`, `trading-dispatch-world-gate.ts`, `trading-gate-block-store.ts`) already reads the ingested world calendar's `days_to_earnings` per symbol.
...
The ticket reads its funds from GET /account, which now also answers `settlement`. On a CASH account under an armed policy the ticket sizes '% of available' against SETTLED cash, the context line reads 'Settled to spend: $X · $Y unsettled, settles <day>', the summary row is labelled 'Settled cash', and the funds advisory names the unsettled proceeds and their settlement day. POST /decisions/manual runs the same kernel check after the guardrails (WHY before confirm; the engine re-checks at execution): a BUY funded by unsettled proceeds is 422 settlement_blocked { message, settlesOn, settlement } under 'refuse' or a `warning` under 'warn'; a SELL of a symbol bought while proceeds were unsettled carries the good-faith-violation advisory as a `warning` — never a block (a failed recent-buys read is logged and the advisory is simply absent). Warnings merge with the protected-entry scheduler warning (' · ') instead of overwriting it. The account header shows the CASH/MARGIN pill from the book row and, on cash accounts, an 'Unsettled buys' select (server default / refuse / warn only). Known cosmetic edge: a PAPER Alpaca account that reports multiplier '1' is typed cash by the adapter, so GET /account's view labels the ticket 'Settled to spend' / 'Settled cash' (source 'ledger', settled = cash minus this book's unsettled sells) while the engine guard no-ops for a typeless paper book — the labels are accurate, the refusal path is not armed; harmless and only reachable on a non-default Alpaca paper configuration.

### D4. Dated/timed orders — SHIPPED v1 (2026-09-04)

As built: `src/app/trading-dated-orders.ts` (kernel) stores one FORCE-RLS row per timed order — the operator's already-minted `operator` decision plus the fire instant — and `dispatchTradingEventSchedule` ticks them on the **same** per-user `trading-events:<sub>` leg the IPO playbooks and protected lots ride (the leg fires every minute, 07:00–19:59 ET weekdays — `TRADING_EVENTS_CRON`; plans step on its full tick, every 5 minutes by default — `TRADING_EVENTS_FULL_TICK_MINUTES`; gated by `TRADING_EVENT_PLANS`). No new schedule type, no per-order schedule row, no cron change: the design's `once: true` schedule became "a row the existing leg fires once", which is smaller and needs no pause semantics. A due row fires exactly once through `deps.place` → `placeDecisionOrder` (guardrails, live gate, reservation arbiter unchanged; ONE requestId `dated-<id>` per decision, so a retry can never double-place). Safety: a window missed by more than `TRADING_DATED_LATE_MINUTES` (default 20) **expires** unfired — never a stale order; a closed market at the fire time (an exchange holiday) expires it too; an engine refusal is terminal (`error`, no retry). Fire times are Eastern wall-clocks converted by round-trip (no DST table) and refused when in the past, beyond `TRADING_DATED_MAX_DAYS` (default 30), a weekend, outside 09:00–16:55 ET, or off the 5-minute grid — what you pick is when it fires. Store (trading 1.9.2): `POST /decisions/manual` takes `fireAtEt {date,time}`, validated at parse time with the leg ensured *before* the mint (a 503 changes nothing), and answers `dated`; the ticket's step 2 asks Now / At a time (ET), a live timed order confirms before the mint (there is no Place step), step 3 shows Scheduled; `GET /dated` + `POST /dated/:id/cancel` back the account page's Timed orders card. Protection rides along: the lot's 2-day unfilled-entry release counts from the fire time (`notBefore`). Follow-up (kernel, 2026-09-06 — `trading-dated-orders.ts` SEQ 2, `trading-event-plans.ts` SEQ 3, `schedule-runtime.ts` SEQ 7, `src/features/trading/services/nyse-holidays.ts`): the `trading-events:<sub>` leg now fires **every minute across the extended session** — `TRADING_EVENTS_CRON`, default `* 7-19 * * 1-5` in America/New_York. 07:00 is where Schwab's SEAMLESS session opens, so a live timed order is never accepted for an hour the venue cannot trade; Alpaca paper could start at 04:00, and the env can widen it. Dated orders tick on **every** fire (minute precision: a fire lands within the runner's poll of its chosen minute — `SCHEDULER_POLL_INTERVAL_MS`, 15 s code default, **60 s in the operator compose**, verified in the running api); event plans and pinned lots step only on a **full tick**, the fire minute divisible by `TRADING_EVENTS_FULL_TICK_MINUTES` (default 5) — stateless and minute-aligned (`isFullTick(scheduleFireInstant(schedule))`, judged on the schedule's due minute, not the poll clock), so an api restart or a second replica can neither double-step nor skip a step, and a plans/lots failure no longer skips the dated tick. The accepted fire window is **derived from the leg cron** (`legWindowFromCron` → `datedWindow()`: 07:00–19:59 ET by default; the v1 `*/5 9-16` derives to the old hand-typed 09:00–16:55) — there is no second window setting to drift. Outside the regular session (`TRADING_DATED_REGULAR_ET`, default 09:30–16:00) `validateFireAt` accepts only a LIMIT + extended-hours + DAY order — that is the venues' rule, not an adapter's (Alpaca honours `extended_hours` on a LIMIT; Schwab's SEAMLESS session; both adapters pass the TIF through) — and it **fails closed** when the caller gives no order shape, so the trading 1.9.2 store, which still calls `validateFireAt(fireAt)`, can schedule regular-session times only until it passes the shape (store 1.10.0). NYSE full closures are refused **by name** at scheduling (`The market is closed on Mon, Sep 7, 2026 (Labor Day).`) from a static 2026/2027 table verified against the exchange's published calendar; `TRADING_MARKET_HOLIDAYS` (`YYYY-MM-DD[=Name],…`) adds one-off closures; `tests/unit/nyse-holidays.spec.ts` goes red 60 days before the table's horizon so it is refreshed on purpose, never silently stale; no external fetch. Early-close days are deliberately not modelled: a 15:00 fire on 2026-11-27 is accepted and the venue's session at fire time decides (`post` places, `closed` expires — both pinned). Existing legs migrate at scheduler boot: `migrateEventLegSchedules` (schedule-runtime.ts, sequenced **before** `runner.start` so the stale-repair pass cannot overwrite it) re-issues `createSchedule` for every `trading-events:*` row whose cron/timezone differ — create-or-replace keeps the id, status, executionCount and lastRunAt and re-scores the Redis next-run index (proven against the real Redis store); a paused leg stays paused; the store's arm route also calls `createSchedule` on arm, so arming self-migrates. `TRADING_EXTENDED_HOURS` gates the autopilot's market→marketable-limit conversion only; a dated extended-hours LIMIT places with the flag off (engine line pinned). Cost, stated plainly: 780 leg fires per user per weekday at the default window (v1: 96), each non-full fire one Redis pop + one indexed SELECT on `oshal_trading_dated_orders` (the schema bootstrap is memoised per process — it used to run its DDL on every tick); **156 full ticks per user per day (v1: 96)**, so EDGAR polling for a `watching` plan and broker `getOrder` polling for working entries/exits and pinned lots rise 1.6× (2×, 192/day, at a 04:00 start). Runner bound: `DUE_BATCH_LIMIT` 20 per poll, `SCHEDULE_MAX_INFLIGHT` default 48 (unset on the box); every user's leg is due at the same second each minute, so with N per-minute legs the runner keeps up while N ≤ 20 per poll cycle (≤ 80 legs/min at the 15 s default, **≤ 20 legs/min at the operator's 60 s poll**; beyond that the due backlog grows every minute and 'minute precision' degrades toward the 20-minute late window, after which dated rows expire unfired). Today the box carries one leg.
...
### D5. Event rules — earnings reaction — KERNEL SHIPPED v1 (2026-09-06), surface not built

As built (kernel): `src/app/trading-earnings-rules.ts` owns a FORCE-RLS `oshal_trading_event_rules` table — one row per (book, symbol, earnings) with beat/miss/inline actions, sizing, the expected print date and an expiry — and a state machine (armed → detected → classified → fired | fired_short | no_action | expired | cancelled | error) that rides the existing `trading-events:<sub>` leg on its FULL tick, gated by `TRADING_EARNINGS_RULES` (default false) on top of the leg's own `TRADING_EVENT_PLANS`.

Scope is deliberately narrow: only symbols HELD on the book, only inside the earnings window, one EDGAR submissions read per held symbol per tick — never a market-wide crawl. Detection is a new 8-K carrying item 2.02, keyed on the acceptance timestamp (the only field that makes "new" meaningful for a release that lands at 16:05 ET). The primary document is read by the accountable trading-analyst through the existing bot path, so the cost lands in `chat_tasks`. Beat/miss/inline is judged against the company's OWN filed numbers — the prior-year period and its own prior guidance — never Street consensus; that sentence is exported as a constant and appears verbatim in both the prompt and the decision rationale, and `unclear` maps to hold: an unread filing is never a trade.

Two guards live in the module because the ENGINE does not cross them for this shape of order, both verified in the engine source: `placeDecisionOrder` never reads `TRADING_HALT`, and the notional guardrail is skipped when the reference price is 0 — which it is for a MARKET order. So the rule honours the halt itself, sizes against the fleet caps, and fires a marketable LIMIT so the engine's own check applies too.

Two failure modes are handled explicitly rather than left to chance. **Truncation is never silent:** the fleet caps bound what ONE order may carry, so a whole-position exit is completed in tranches, and any rule that ends short of its intent — capped, tranche-bound, a stale verdict, a faded reaction, or a terminal refusal — ends in the distinct status `fired_short` carrying a `shortReason`, the exposed share count on the row and in the timeline, and a WARN. `fired` means the intent completed; `no_action` means no share moved. **An ambiguous submission never becomes a second fill:** the engine releases its reservation in the catch and the venue ignores our client order id, so before anything is re-placed the venue's own order record is consulted — a matching order is adopted, a BUY is never re-placed on any answer, and an unanswerable venue is terminal with "check the account". The attempt bound is per rule, not per tranche.

Not built: the surface — rules are created and read through the kernel module only, so nothing in either repo can arm one today. That absence is a MISSING FEATURE, not a safety control, and must not be relied on as one: the tick is wired into the leg and the live gate on the operator's box is already armed, so `TRADING_EARNINGS_RULES=true` is the only remaining switch between an armed rule and a real venue. The feature has also never executed against a real broker — every venue seam in its guard suite is doubled — so paper first, and consider an attempt bound of 1 for a first live exposure.
