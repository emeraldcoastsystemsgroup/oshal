...
Not built: the surface (rules are created and read through the kernel module only), and the feature has never executed against a real broker — every seam in its guard suite is doubled, so paper first.
