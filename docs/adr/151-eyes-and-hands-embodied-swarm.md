...
| The **hands as a parts model**: a printed six-axis arm sized from one servo class, its parts CAD Studio programs, its MuJoCo model generated from the same design, and the sizing checked against the physics rather than asserted | ADR-152 D1/D5 task 3, store package `embodied` 0.12.0 (`/build/arm`, `/physics/arm/check`) | The taught pick-and-place is the baseline a learned grasp must beat (embodied BACKLOG B21); the arm reaches the room's world and the rail through B22. |
