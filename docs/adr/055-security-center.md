...
- **Status:** Accepted — implemented (reconciled 2026-07-31) as the `security` kernel app: `swarm-apps/security.yaml` (Security Center — deterministic scanners on the controller, reasoning on the security-analyst bot, which moved from inline-on-the-api to its own bot node on 2026-09-16 — see [the delegation routing table](../security/http-delegation.md) — with its reason-only scope unchanged) + `src/features/security/`; scope as decided — observe + triage + escalate-to-ticket, no auto-remediation
- **Date:** 2026-06-19
