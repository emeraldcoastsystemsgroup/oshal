...
- The repeatable acceptance harness uses mock authentication and seeded records on an isolated local database. Live rollout was subsequently verified as recorded below.
...
- Slice 2a is implemented in the store: Feeds reads indexed Slack counts and recorded sync time; Intelligent Communication reads the caller's saved encrypted email digest; Switchboard reads current publishing/reply-outbox states. The package ledger defines their metrics, owner scope and freshness. All use standalone session-authenticated read-only routes.
- Slice 2a validation: TypeScript, 12 compiled-route boundary tests, catalog/manifest checks, actual PostgreSQL schemas with owner RLS and SELECT-only source grants, repeated GETs, partial failures and missing/corrupt digest keys pass. Chromium verifies the three boxes, metric hiding and persistence, related-highlight exclusion and desktop/mobile layout. Authentication and source rows are test fixtures, not live accounts.
- Live rollout completed 2026-09-10 UTC after operator approval: core #411 and store #153 merged; the sanctioned deploy shipped core `7aa6f62f` with migration 126 and all 35 app containers healthy. Identity 1.1.0, Feeds 1.1.0, Email Summarizer 1.1.0 and Switchboard 0.5.0 were installed from store commit `c1817859` and hot-loaded through the normal app loader.
- Live verification: preferences return 200; all four summary endpoints return 200 without partial-source failures; the Home plan discovers all four metric catalogs; Chromium renders all four in the actual authenticated cockpit using existing account data. Temporary verification credentials were revoked. This proves saved-data extraction and rendering, not current provider liveness.
- Rollout also exposed and fixed the private-store installer's unauthenticated lazy blob fetch during sparse/audited checkout (core #413). Its merged CLI was run from an exported tool snapshot for these installs. The configured public snapshot catalog still has older versions, so the four installs explicitly selected the applications repository.
- Slice 2b (cached priority/social coverage) and slices 3-6 remain planned; time-window selection and cross-app semantic event deduplication are not implemented.
