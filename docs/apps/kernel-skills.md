...
| `bound-workflow-results` | `@/features/swarm-orchestration` | Compatibility floor for evidence-bound, reason-only queued work. The controller injects domain admission/result validation while the canonical dispatcher retains signed transport, configured owner brain and task accounting. |
| `futures-forward-receipts` | `@/app/trading-futures-prediction-ledger` | Owner-scoped immutable forward research inputs, database issuance and same-contract outcome grading. Requires migration 161; console packages refuse older runtimes rather than losing the ledger boundary. |
