...
**Installed/source distinction (2026-09-26):** The operator's active data direction is Schwab,
not a Kibot purchase or mount. Schwab dated-contract forward capture is ON, with 220 ESZ26 and
219 CLX26 closed bars at the last signed-in check; the owner-facing two-day catch-up preview
and confirmation inserted zero duplicates. The console also offers **Schwab private captured
bars** as a research source, but no Futures study schedule is configured and this current-contract
slice does not supply historical roll overlap or a valid nightly/OOS run. The file/API archive
path documented below remains an implemented optional source; Kibot itself is only a roadmap
discussion. A one-time historical backfill from an operator-chosen source is still needed.

## Current path

The Trading console's Strategies → Tuning panel stores one owner-scoped `trading-futures-research:<sub>` schedule. The operator chooses roots, supported chart/higher timeframes, a source (file/API archive or owner-private Schwab captured bars), source-specific settings, start date, end policy, walk-forward split, stage grids and a UTC cron. A container-visible data directory applies to archive sources, not Schwab. The default `latest` policy resolves the end to the previous completed day on **each** run under the chosen source's calendar; `fixed` preserves an explicit historical end. The resolved end is recorded with the run for reproducibility. The route is operator-gated for create, run, pause, resume and delete. Stopping the stock advisor does not stop Futures. An installed source selector is not evidence that an admissible Schwab history or actual nightly schedule exists.
