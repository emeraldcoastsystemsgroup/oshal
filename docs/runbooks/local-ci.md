...
|---|---|---|
| head-src (`--head`/`--scheduled` only) | clean `git archive HEAD` export + `npm ci` — unattended runs judge committed code, never a mid-edit tree | yes |
| typecheck | `npm run typecheck` | yes |
| unit | `npm run test:unit` (vitest, no DB) | yes |
| lint | `npx eslint src/` | no — advisory (retired-CI parity) |
...
| secret-scan | dockerized gitleaks (`--network none`) on a **HEAD export**, `.gitleaks.toml` allowlist | yes |
...
| image-build | `git archive HEAD \| docker build` → scratch tag `oshal-ci:latest` (never touches `any-bot:latest`) | yes |
...
```bash
bash scripts/ci-local.sh --head          # what the daily task runs (minus the alert): committed HEAD
...
Windows task **"OSHAL Local CI"** runs daily at 10:00 local, windowless, via
