...
- [multi-account-books-cutover.md](multi-account-books-cutover.md) — ADR-134 operating + cutover runbook: the operator's path from one Schwab login to N account-scoped trading books (re-connect with all accounts consented → discovery → `scripts/trading-books-cutover.sh --arm` → create/arm books), day-2 rules (disable ≠ abandon, immutable bindings, breaker resets), and the post-cutover flag-off + roll-forward rollback doctrine.
- [dev-box-disk-reclamation.md](dev-box-disk-reclamation.md) — why C: fills up on the dev box (Docker Desktop keeps everything inside one `docker_data.vhdx`, which grows but never shrinks on its own), what was taking the space on 2026-09-20 (three orphaned `oshal_restore_smoke_*` databases, 305 anonymous volumes leaked by the disposable-postgres test fixture), the scoped stage-one reclaim that was run and the global prunes that must never be, the stop-first `diskpart` compaction (stage two, needs Docker fully stopped; `Optimize-VHD` is not on Windows Home), and the monthly check.
