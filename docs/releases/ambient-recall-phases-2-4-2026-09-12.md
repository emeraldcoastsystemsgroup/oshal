...
| `scripts/person-model-gate-in-container.js` | the same assertions as the spec, as plain Node | run inside `oshal-local-api` when the host cannot reach Postgres |
...
5. **If the host cannot reach Postgres:** copy the two scripts into the api container and run them there:
...
docker exec oshal-local-api node /tmp/pm-gate/gate.js          # scratch database, dropped afterwards
MSYS_NO_PATHCONV=1 docker cp scripts/person-model-live-proof.js oshal-local-api:/tmp/pm-gate/live.js
docker exec oshal-local-api node /tmp/pm-gate/live.js          # synthetic owner, removed afterwards
```
