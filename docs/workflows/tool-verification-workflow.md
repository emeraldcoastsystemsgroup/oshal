...
The tool verification workflow allows administrators to validate that registered tools are properly installed and responsive. Verification can be triggered manually (single or batch) or automatically via the scheduler.
...
| `SKIPPED` | ⚪ Gray | Tool was skipped (disabled) |
...
| Single tool FAILED | Tool endpoint down | Check tool service status |
