...
| `tests/unit/monitoring-liveness-watch.spec.ts` (BUG-21 tail: nothing ran the strict monitoring check unattended) | Scoped doubles OUTSIDE the boundary: the `docker` CLI, shimmed on PATH so alert delivery is asserted as exact argv rather than exec'ing a live container, and — in the recovery/streak cases only — a stand-in check whose exit status the case chooses, used to drive the watch's own state machine | REAL: Git Bash running the shipped watch and the shipped `monitoring-liveness-check.sh --strict` against a deliberately closed TCP port (real `curl`, real refused socket, real exit 1 naming the endpoint), the real on-disk streak/state and log files across consecutive runs, `cscript.exe` executing the windowless launcher next to a stand-in watch (exit 37 propagated), and `powershell.exe` executing the registration script's dry run. NOT evidence for the engine-restart boundary: the Docker engine may not be stopped on the operator's box, so the stop-the-engine reproduction the BACKLOG done-when asks for remains unrun | Green 2026-09-15 |
