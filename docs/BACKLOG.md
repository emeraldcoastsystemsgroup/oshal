...
### Application groups — stage 1: the `kind: group` manifest, borrowed surfaces, `readiness:`, one setup dashboard (ADR-141)
- **Remaining:** design accepted as an ADR only. Core: the manifest kind (no code allowed), `toolbar[]` resolved by app + surface name into the synthesised ribbon profile (fail closed on a missing member or surface), the per-user `readiness:` schema on member manifests (session-authenticated, below the member's own mount, RFC 6901 pointers like `smoke:`), one kernel-served setup dashboard rendered from `setup[]` that never renders "probe unavailable" as done, and activation rules (members installed + active, no nesting). Store: the `intelligent-career` group package, `readiness:` entries in career-hunter / portrait-studio / social / print-ingest, an ADR-139 `artifacts:` destination on career-hunter for "Add career materials", and `HOST_APP_MAP` pointing career.oshal.ai at the group.
- **Done when:** a new user landing on career.oshal.ai sees the seven setup steps with live status, completes them from the dashboard alone, the toolbar is the union of the four members' surfaces with nothing hand-copied, a group whose member is uninstalled refuses to activate naming the member, and `tests/unit/` carries a resolver + dashboard guard over a fixture group (missing member, renamed surface, unavailable probe).
