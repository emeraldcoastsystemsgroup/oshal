...
**277 entries open.** 18 closed and recorded in the ledger since it began on 2026-09-20.

| Status | Count | Meaning |
|---|---|---|
| IN PROGRESS | **0** | being worked in the current session |
| OPEN — actionable | **106** | no decision, no live box needed; can be closed by an agent |
...
| OPEN — untriaged | **47** | filed after the 2026-09-15 triage; has no verdict yet |
...
| Date | Entry | PR |
|---|---|---|
| 2026-09-23 | Refusal visibility: the substrate for P1, P3 and P4 | #798 |
...
### PICK UP HERE — exact handoff from the 2026-09-23 backlog run
- **Status:** OPEN — actionable

**Read this first.** The box is healthy and the fixes below are already live from reviewed preview
heads; the remaining work is to land those exact changes in the right order, align the deployment
with merged `main`, and finish the two advisory app/bot pins. Do not restart discovery from the old
2026-09-21 handoff or recreate completed backfills.

**Live state at stop:** core revision `94d1d2b2b0c0daa5cea17873eaf2b2a2d3dc5e75` on image
`e0a4645a7be6`, concierge mode `enforce`, API `RestartCount=0`, 37/37 app containers healthy,
83/83 manifests loaded with zero failures, zero concierge-coverage warnings and zero authorization
catalog migrations. The supported post-deploy rerun passed bot-role grant, Jarvis answer and ticket
dispatch. Social is active and both `communications-bot` and `social-writer` are active. The live
app/bot integrity guard reports PASS with two advisory MISPINs (`brand-graphics`, retired `trading`)
and 19 deliberate shares.

- **Remaining:** land the reviewed heads, reconcile the deployment with merged `main`, and clear the
  two advisory bot pins. The PRs are stacked, so use this order:

1. **Core #798 first.** It contains the P1/P3/P4 refusal work already recorded in the closed ledger
   and is the base branch of #799. Its exact work was live-proven before this handoff.
2. **Retarget/recheck core #799 against merged `main`, then merge it.** Its deployed implementation
   commit is `94d1d2b2`; the remote publish gate passed there, the forced-enforce suite passed
   257/257, the broader final audit passed 128/128, and the Social lifecycle repair is included.
   This handoff is a later docs-only commit on the same PR, so require the publish gate on the PR's
   then-current exact head too.
3. **Merge store #252.** Its reviewed head is `c7def83`; it holds the 13 concierge backfills. This
   must precede refreshing #249 because it advances Create to 1.8.2 and its audit to 1.8.2.
4. The operator explicitly authorized a maintainer/admin bypass because this repository has one
   maintainer. GitHub cannot record an author's self-approval: use the admin merge path and record
   the bypass honestly, never manufacture a second reviewer. Re-run each exact-head gate before
   merging if its base or head changed.
5. Deploy the resulting core `main` with concierge mode explicit `enforce`; repeat exact revision,
   83/83 load, 37/37 health, restart-count, Social worker, zero-warning, Jarvis and ticket proofs.

**Then finish the integrity item:** the prepared store worktree
`C:\Projects\oshal-applications-pr249-tmp` is clean at local head `51f97dc`, but it is **not pushed**.
It repairs #249's missing Brand Graphics audit/README changes and the two audit drifts found on the
then-current store main. After #252 lands, refresh this branch against the new store `main` and keep
Create's newer **1.8.2** audit — do not blindly push the prepared 1.8.1 record. Re-run catalog,
package-audit, Brand Graphics validation and the full store gate; then push/admin-merge #249,
install Brand Graphics 1.1.1 while preserving its inactive state, restart the API for boot
reconciliation, and verify its version, inactive status, exact install SHA and corrected bot id.

Finally delete only the retired `trading` registry row after the full fail-closed predicate in its
own entry matches exactly one row; never touch `intelligent-trades` or a trading business table.
Re-run the integrity guard: it must have no MISPINNED section and must retain the deliberate-share
informational line. After that, the next recommended backlog order is Rides #243 (including the
missing Compose passthrough), then reconcile the dirty second-leg trading PR #244 without arming a
book.

- **Done when:** #798, #799, #252 and #249 are landed in the order above; merged `main` is the exact
  healthy deployed source; Brand Graphics 1.1.1 is installed on its own id; the one retired trading
  row is gone; the integrity guard has no MISPINNED finding; and this pickup entry is deleted rather
  than becoming another stale handoff.
...
### Automated post-deploy product proof must remain binary under delegation signing (2026-09-16)
- **Status:** OPEN — actionable

- **Current state (live-proven 2026-09-23):** this is no longer permanently UNVERIFIED. An
  issuer-carrying operator PAT is available to the deploy environment. On the `94d1d2b2` deploy,
  bot-role grant and ticket dispatch passed while Jarvis's external provider returned
  `unauthorized` and then an upstream 503; the deploy command correctly returned nonzero. The
  supported no-redeploy rerun then passed all three checks: bot-role grant, a real Jarvis answer,
  and general-bot moving an approved ticket to complete. The remaining work is landing #798 first
  and repeating that binary proof from merged `main`, not obtaining another credential.
...
- **Why automation could not fix this from inside the box at the time.** The check could only mint a service-secret PAT,
  ...
  `{"success":false,"error":"delegation_required"}`, HTTP 401. At that point, automation on the
  deployment could not prove that Jarvis answered.
...
- **The historical unblock was one operator action, now completed:** a PAT minted from a signed-in
  session records its issuer; `OSHAL_VERIFY_OPERATOR_PAT` now reaches the deploy verifier without its
  value being logged.
- **Original done-when, now met on the preview deployment:** `jarvis-ask` and `ticket-dispatch`
  report PASS or FAIL rather than UNVERIFIED on a
...
- **The exit-contract implementation is on #798 (open, live-proven, not merged).**
...
- **Remaining:** merge #798 first through the operator-authorized maintainer/admin path, deploy the
  resulting `main` in the ordered handoff above, and repeat the three checks. The separate cleanup
  defect below does not change the product verdict, but it must be fixed so each proof removes its
  own synthetic conversation rows.
- **Done when:** the three binary product checks pass on the exact deployed merged-`main` revision
  and any failed check still makes the deploy exit nonzero. Remove this entry then; the durable P4
  evidence already lives in the closed ledger and named tests.

### Deploy verification cannot clean its Jarvis task while `oshal-engineering` is inactive
- **Status:** OPEN — actionable

- **Measured twice on the 2026-09-23 deploy:** after closing and deleting its synthetic chat ticket,
  `scripts/operations/deploy-live-verification.js` calls `DELETE /api/tasks/<deploy-verify-id>`.
  The app gate assigns that route to `oshal-engineering`; because the app is intentionally inactive,
  cleanup receives HTTP 503 `Application inactive`. A passing Jarvis proof therefore still prints
  `CLEANUP FAILED` and leaves its `chat_tasks` row plus messages behind.
- **Tonight's residue is gone:** failed thread
  `deploy-verify-9278c0ec-4d5c-475c-93ab-59974c0acee0` and successful thread
  `deploy-verify-af94e5f8-79ac-4f5c-ae71-36ace04b2d61` had zero ticket links and were deleted by exact
  id in one count-guarded transaction; the post-check returned zero rows. No user conversation was
  touched.
- **Remaining:** give the authenticated verifier a cleanup path whose ownership remains valid when
  optional engineering surfaces are inactive, or move task deletion under the kernel owner that
  actually serves the task API. Do not solve this by activating an unrelated app or weakening the
  app gate globally; keep owner/auth checks and exact task scoping.
- **Done when:** a regression runs the real cleanup sequence with `oshal-engineering` inactive,
  receives success, and proves the exact synthetic `chat_tasks`, `chat_messages` and ticket links are
  absent afterward; a failed or foreign cleanup remains refused.
...
- **Which makes this a migration, not a gate.** Switching on load-time validation would refuse 26
  ...
  touch core.
- **Sequencing (unblocked 2026-09-23):** the refusal ledger, reviewed remedies, and terminalization
  path are live. Take this next as warn-on-load, store-repo backfill, then fail-closed; no allowlist.
- **Implemented and live-proven 2026-09-23, but not landed on `main`:** core PR **#799** contains
  deployed implementation commit `94d1d2b2b0c0daa5cea17873eaf2b2a2d3dc5e75`; store PR **#252** is at
  `c7def83df4f38dfe698a752504a09af1f79aecb5`. The core now defaults coverage to `enforce`
  (explicit `warn` remains a rollout escape), and the store backfills the 13 surfaced packages that
  were missing a concierge. The forced-enforce core sweep passed 17 files / 257 tests; the broader
  final Social/P8 review passed 14 files / 128 tests; the real-PostgreSQL integrity guard passed 5/5;
  typecheck, targeted lint and manifest validation passed. #799's remote publish gate passed at
  `94d1d2b2`; this later docs-only head must pass it again. GitHub reports no branch checks for
  #252, whose local store validation passed.
- **The deploy found and fixed the last adjacent stop-line:** `social` declares
  `communications-bot` locally and a distinct `workflow.workerBot: social-writer`. Lifecycle
  reconciliation used to return as soon as it saw the declared bot, so an active Social app could
  leave `social-writer` inactive forever after a restart. Core commit `94d1d2b2` unions declared bot
  ids with only the canonical external workflow worker; it deliberately does **not** union durable
  `agent_ids`, because that column also contains borrowed/shared associations. The regression uses
  three distinct ids and proves the declared bot plus worker activate while a borrowed `chatBot`
  is neither looked up nor toggled.
- **Exact live state after deploying `94d1d2b2`:** image `e0a4645a7be6` reports the exact revision,
  `OSHAL_CONCIERGE_COVERAGE_MODE=enforce`, API `RestartCount=0`, 37/37 app containers healthy, and
  auto-load `loadedCount=83, deactivatedCount=0, failedCount=0`. Since boot there are zero
  `swarm_app.concierge_coverage_missing`, zero `authorization_catalog_migration_required`, and zero
  manifest-load failures. `social` is active and both `communications-bot` and `social-writer` are
  active. The live integrity guard changed from FAIL to PASS. The first Jarvis probe hit an external
  provider `unauthorized` result followed by an upstream 503; the deploy correctly returned nonzero.
  The supported no-redeploy rerun then passed bot-role grant, Jarvis answer and ticket dispatch.
- **Remaining:** #799 is stacked on #798, not `main`. Merge core #798 first through
  the maintainer/admin path the single operator explicitly authorized on 2026-09-23, retarget and
  re-check #799 against the resulting `main`, merge #799, and then merge store #252. GitHub cannot
  record an author's self-approval, so record the bypass honestly rather than fabricating a second
  reviewer. Deploy the resulting core `main` head with coverage still explicit `enforce`, then repeat
  the exact revision/env, 83/83 load, 37/37 health, Social worker, zero-warning and product-verifier
  checks above. Do not redo the 13 package backfills unless catalog versions prove they are absent.
...
- **Operator correction (2026-09-21):** the original framing was wrong and is retracted.
  ...
  `test-gate-flow` remains, is active, and is correctly associated with `code-reviewer`. The live
  guard now reports **19** associations that point at a bot another app owns without a declared-name
  mismatch; these are deliberate shares, including
...
- **Remaining:** two wrong pins, and only two. A wrong pin is not a shared association; it is a
  ...
  - `brand-graphics` declared a bot named `brand-graphics` under `b00f0000-…-0001`, which is the drone
    package's `drone-operator`. The UUID repair in store PR **#249** uses `b0110000-…-0001`, which was
    unused before this repair and remains absent from the live `agents` table, but remote PR head
    `f41603e` is not ready
    to install: it leaves `audits/brand-graphics.json` at 1.1.0 and the package README on the stolen
    id. A clean local continuation fixes both and is recorded below. Version 1.1.1 has not been
...
- **Live state after the Social repair (2026-09-23):** the guard reports `RESULT: PASS`, not an
  outage, with exactly these two MISPINNED advisories and 19 deliberate shares. `brand-graphics`
  remains inactive at 1.1.0; `intelligent-trades` remains a separate active row on the correct
  `a0000000-…-0046`; and no live registry mutation for this item has been performed.
- **Prepared continuation, not pushed:** clean worktree
  `C:\Projects\oshal-applications-pr249-tmp` is on `fix-mispinned-agent-ids` at `51f97dc`.
  It adds `audits/brand-graphics.json` 1.1.1 and the corrected README id (`c6f1622`), merges the
  then-current store `main` (`5f7fbd1`), and repairs the two audit-version drifts found there for
  `create` 1.8.1 and `intelligent-trades` 1.19.1 (`51f97dc`). Catalog and compatible-audit checks
  pass for all 61 packages, the catalog/audit suite passes 17/17, `oshal-app validate
  brand-graphics` passes with zero warnings, and the full local store gate reports 55 passed,
  0 failed, 2 documented environment-only partials (Career disposable-PostgreSQL/symlink and the
  Embodied live engine), 0 skipped and 0 blocked under `--allow-skips`.
- **Next developer starts here:** merge store #252 first, then refresh the existing #249 branch from
  the prepared worktree against the newer store `main`; #252 advances Create to 1.8.2, so preserve
  its 1.8.2 audit rather than blindly pushing the prepared 1.8.1 record. Re-run the gates, push that
  branch, and use the operator-approved maintainer/admin merge path for #249. Install
  `brand-graphics` 1.1.1 from the resulting store `main` through the canonical
  `scripts/oshal-app.js install` path with the shared workspace volume mounted, preserving its
  inactive state. Restart the API so boot reconciliation updates the row, then prove version 1.1.1,
  inactive status, exact install SHA and association `b0110000-…-0001` while active
  `drone-operator` remains `b00f0000-…-0001`.
- **The legacy-row deletion must remain count-guarded and exact:** the one candidate is app id
  `59fc9ea6-a387-4d78-82c8-b26c2d0aa005`, name `trading`, version `1.0.0`, status `inactive`, path
  `/app/swarm-apps/trading.yaml`, no owner, and sole agent id `a0000000-…-0045`. It has zero
  `oshal_app_access` rows and no trading business table references the registry row. In one
  transaction, assert that the full predicate (including the manifest's declared bot name/id)
  matches exactly one row before deleting exactly one; otherwise roll back. Do not touch
  `intelligent-trades` or any trading ledger table.
- **Done when:** `bash scripts/swarm-app-bot-integrity-check.sh` reports no MISPINNED findings — which
  ...
  deliberate shares stay a counted informational line rather than findings. The guard for the shape is
