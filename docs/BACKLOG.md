...
- **Source proof:** when `resolveUserLlmConnection` returns nothing and the bot's configured harness is an unbrokered CLI, the `/ask` turn records `Jarvis has no AI engine connected — add one under Settings → Connections → Bring Your Own LLM.` with code `NO_HOSTED_BRAIN`, and the surface speaks that state instead of "Sorry, that didn't work". The pointer names the card that exists on the Connections tab. [The guard](../tests/unit/jarvis-no-hosted-brain-honesty.spec.ts) drives the real router, the real execution chokepoint and the real bot registry, proves the work-queue/briefing shelf still lists its rows in the same state, and evaluates the spoken line out of the shipped `jarvis.html`. Root cause 2026-08-09 (the operator's `any-llm` row had disappeared, the operator is by design exempt from free-tier fallback, and SEC-05 preflight correctly refuses every CLI harness at bot nodes) is unchanged: this fixes what the operator is told, not what is refused. Same honesty doctrine as the PR #54 voice fix.
- **Remaining:** confirm the written sentence and the spoken line on a deployed box whose ladder is genuinely empty — the guard crosses the real modules but not a browser or a live speech engine.
- **Done when:** an operator with no connected endpoint reads and hears the same actionable sentence in the browser, and the work queue still lists its items.
