...
- **Delivered (0.5.0, 2026-09-14):** a local electromechanical lab — ngspice in a package-owned engine container, a schematic canvas whose gears, rotors, servo horns and crank-sliders move from the solved angles, a breadboard view held to the schematic's nets, DC motors, hobby servos and steppers with their gear trains, torsion springs, slipping belts and a crank-slider all in the same SPICE solve, thirty parts, a shaft-driver catalog, the gear → CAD Studio hand-off, thirteen route-backed tools for the inline concierge, seven suites registered in the Test Lab, and the operator's external-tool list evaluated under the package's `docs/evaluations/`. See [ADR-154](adr/154-local-electromechanical-lab-circuit-lab.md); the package BACKLOG carries B1 firmware, a general 2-D linkage layer (B5's residual) and the evaluation follow-ups B12–B16.
