...
- **September 13 source checkpoint:** compact Home/Jarvis, named Finance and the OSHAL application menu are implemented with registered browser and lifecycle tests; installed acceptance is pending. See [Finance and the OSHAL menu](backlog/cockpit-workspace-navigation.md#requested-follow-up-finance-and-the-oshal-menu). Remaining producer and context-action work is tracked below.
...
- **Reconfirmed 2026-09-13, P1:** the Jarvis layout is still unsatisfactory. Keep the assistant compact, show updates/details together, make Finance a named area and move miscellaneous tools into the OSHAL menu instead of the default dashboard's `Other` section. Consolidated into JDX-01/JDX-02 rather than a second redesign backlog.
...
- **September 13 source checkpoint:** compact composer and explicit assistant state, bounded source/topic groups, adjacent named daily areas and selected details, preserved composer during Home redraws, and a searchable authorized directory. AI Test Lab registers the actual suites separately from installed-asset readiness. Installed acceptance is pending.
...
- **September 13 source checkpoint:** admitted Finance is a top workspace and named daily area; miscellaneous apps are searchable in the brand-adjacent OSHAL menu. Only explicitly delegated sidebar entries are removed. Installed acceptance is pending.
