...
**293 entries open.** 1 closed and recorded in the ledger since it began on 2026-09-20.

| Status | Count | Meaning |
|---|---|---|
| IN PROGRESS | **5** | being worked in the current session |
| OPEN — actionable | **110** | no decision, no live box needed; can be closed by an agent |
...
| OPEN — untriaged | **52** | filed after the 2026-09-15 triage; has no verdict yet |

**Being worked right now (5):**
- The ONNX runtime installs process-global rethrow handlers that make any stray rejection fatal (2026-09-15) · agent: exported strip helper + host exit case
- A machinery-written `agent_config` row should not outrank a fleet-default write (2026-09-17) · agent: precedence rule + real-Postgres guard
- Warn before an unrenewable connection lapses · agent: implement the pre-lapse warning + guard
- Data-model explorer: export the view you are looking at · agent: close remaining clauses (a) and (b)
- Lazy-DDL trigger and function guards must converge, not create-once · agent: clause-by-clause verification

**Closed most recently** (full ledger: [backlog/closed-ledger.json](backlog/closed-ledger.json)):

| Date | Entry | PR |
|---|---|---|
| 2026-09-20 | partner-app-registration.md covers about a third of the wired hub connectors | #688 |
...
- **Status:** IN PROGRESS · agent: exported strip helper + host exit case
...
- **Status:** IN PROGRESS · agent: precedence rule + real-Postgres guard
...
- **Status:** IN PROGRESS · agent: implement the pre-lapse warning + guard
...
- **Status:** IN PROGRESS · agent: close remaining clauses (a) and (b)
...
- **Status:** IN PROGRESS · agent: clause-by-clause verification
