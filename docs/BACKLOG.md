...
### career-hunter — user targets: browser closure evidence (1.13.0)
- **Remaining:** 1.13.0 ships each user's own pattern-gated target list (store PR #117) beside the admin's shared `companies` table; the routes, the engine `classify`/`add-target` verbs, migration 100 and the Settings card are deployed and guarded, but the user routes take identity from the browser session only, so the end-to-end path was never exercised from a real signed-in session.
- **Done when:** a signed-in NON-admin user pastes a supported board URL (e.g. a Greenhouse board) in Career Settings → Target companies and the row goes accepted → resolved with a company name and posting count; a generic careers page is rejected with the supported list and no row appears; the Companies admin page shows the `user-added` pill on that employer; and the next evening chain scrapes it (a `first_seen_at` batch for that company). Recorded in `career-hunter/docs/operations.md` §9.
...
### Satellite login push — publish `@oshal/chat` 0.3.0 and prove the browser round-trip (ADR-137 amendment A)
- **Remaining:** core PR #302 gave the node "Log in + push" / "Push to swarm" on the Codex and Claude rows and the controller a demo-gated Claude adoption route, but both known satellites still run the 0.2.0-era build (npm `@oshal/chat@0.2.0` predates it) and the live browser round-trip — vendor CLI login on a satellite, file pushed under the user's OIDC session, swarm bots running on it — has not been exercised.
- **Done when:** `@oshal/chat` 0.3.0 is published (or the archive-push recipe in docs/runbooks/publishing-the-node-package.md is applied to both satellites); from one satellite a "Log in + push" for Codex lands as platform promotion (api log `OpenAI Codex credentials imported from CLI auth.json`, host `~/.codex/auth.json` rewritten) and for Claude as `Claude Code login file adopted (demo portal fallback)`; a non-operator push is refused; and a bot turn afterwards runs on the pushed login.

### This box: codex platform promotion is blocked by an EMPTY `ENCRYPTION_KEY`
- **Remaining:** the api container carries `ENCRYPTION_KEY=` (empty), so `OpenAiCodexOAuthService` refuses every credential operation — status reads log `ENCRYPTION_KEY_REQUIRED` on each boot and a codex push from a satellite would fail the same way (the Claude path is unaffected: it writes the mounted file). Setting a real key touches the live plaintext `/app/output/secrets.json` store, so it is an operator decision, not a drive-by fix.
- **Done when:** either a real `ENCRYPTION_KEY` is set with the existing secrets store migrated and the codex status route stops logging `ENCRYPTION_KEY_REQUIRED` at boot, or the codex import/status routes answer a clear 409 naming the missing key instead of a 500/503 — and `POST /api/openai-codex/oauth/import` from a signed-in operator succeeds on this box.
