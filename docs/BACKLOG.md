...
- **Status (2026-09-18) — the exit is closed at its cause and the done-when is instrumented; the deploy run is still owed.** Root cause, reproduced against a real PostgreSQL (`tests/unit/pool-connection-errors.spec.ts`): pg-pool owns a client's `error` event only while the client is idle in the pool (it removes its listener at acquire, re-adds it at release), and pg routes a server ErrorResponse with no query in flight straight to `emit('error')` — so a client checked out inside `BEGIN`, waiting on a starved event loop, received Postgres's 25P03 termination as an unowned event, EventEmitter rethrew it, and process-crash-guards exited the process. The unowned child in that spec dies with exit 1 and the 2026-09-05 message verbatim (`terminating connection due to idle-in-transaction timeout`, `UNCAUGHT EXCEPTION`). `src/shared/services/database/pool-connection-errors.ts` (`ownPoolConnectionErrors`) now owns every connection's `error` at each pool construction site — api main, the optional-store factory, rag, the two TimescaleDB pools, bot-node, edge — and the same termination is an ERROR line per pg error event (two for one termination: the server’s 25P03 and pg’s own connection-terminated event, milliseconds apart) naming the backend pid, one rejected statement at the call site (`Client has encountered a connection error and is not queryable`, handled by its own catch), and a pool that keeps serving (owned child: exit 0). The recreate pacing is unchanged and is now printed on the deploy's "recreating N bots" line. `scripts/api-storm-probe.sh` (`begin`/`verify`, proven on real containers in `tests/unit/api-storm-probe.spec.ts`) snapshots the api's RestartCount before the recreate, and `scripts/oshal-deploy.sh` refuses to print DEPLOYED — **exit 6**, deployed and serving, not rolled back — when it changed or an `idle-in-transaction` line landed in the window. Which transaction idled is still not identified: the Postgres log for backend 205300 carries no statement (`log_line_prefix` is `%m [%p]`, `log_connections` off) and the api log of that boot is gone; on the dev box today the 60 s `idle_in_transaction_session_timeout` is a role setting on `oshal` only (`pg_db_role_setting`) and the api connects as `oshal_app`, which carries none. The next occurrence is attributable: the owner logs the backend pid and the call site logs its rejected statement. **DONE 2026-09-19.** The deploy ran. `scripts/oshal-deploy.sh` recreated the api and 36 bots in batches of 5, and the probe gated it from inside that window: `api-storm-probe: oshal-local-api RestartCount 0 -> 0; 'idle-in-transaction' log lines since 2026-09-19T15:51:31Z: 0` followed by `api-storm-probe: PASS - RestartCount unchanged and no 'idle-in-transaction' line in the window`. Recorded in [the real-boundary audit](governance/real-boundary-regression-audit.md). One honest limit on what that proves: the window saw no server-side termination at all, so it is the api surviving a storm, not the ownership handler being exercised by one — the handler's evidence stays the disposable-Postgres child, where a real termination is forced and the exit code is the verdict. Which transaction idled is still unattributed, and the next occurrence is what makes it attributable.
