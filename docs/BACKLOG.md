...
- **Remaining:** Wave 1 — destinations: presentations (Office → outline import), little-monsters class materials, print-ingest (text via kernel extract-text); sources: presentations "My files", Jarvis deliverable chips, Video Studio cards. Wave 2 — destinations: dnd character import, youtube-kids Takeout, spaces scan import; sources: career screenshots/PDFs, switchboard compose image, venture-plan exports, dnd/game-show art, task-explorer Files tab, rag-center documents, little-monsters materials/audio. Wave 3 gates: lora needs a real HTTP image-ingest route; client-generated blobs (camera, payroll, ocean-lab, workflow exports) need the mint-with-bytes handle variant. Promote the package-side redeem to a shared helper at the third destination adoption.
