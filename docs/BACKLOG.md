...
- **It never reproduced live, measured on the running box:** 65 `chat_tasks` rows with `metadata->>'origin' = 'jarvis-chat'` spanning 2026-06-20 → 2026-09-15, **zero** with a null `owner_sub`; and `ensureSessionTask failed` — a line the deployed source does carry — appears 0 times in 7 days of `oshal-local-api` logs.
