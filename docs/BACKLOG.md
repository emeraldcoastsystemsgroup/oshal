...
- **Current appearance refinement deployed:** core `2b0dd4fc` makes saved palettes follow shared pages, offers Application colors explicitly, and places optional workspaces in the desktop header or mobile disclosure. Career Hunter 1.20.0 covers thirteen existing screens; Create 1.2.2 covers Home and New design. Checks pass 225 appearance cases, seventeen later focused checks, 71 existing contracts and 28 header/navigation cases in separate commands. Career passes eighteen Chromium checks and 53 Node checks, also repeated through the sealed runner; Create passes nine Chromium and twenty Node/HTTP checks. The standard preview completed with all 35 services healthy and clean image parity. All 278 target package files match; strict preservation passes. Native Chrome verified the header, Career/Create palette switching and Application colors opt-in. Fifteen relevant Lab entries are visible. The actual Lab Run control passed Career's 53 Node contracts with verified cleanup; both core readiness controls passed, including 33 admitted links, without executing their linked browser suites. Exact image, source and acceptance boundaries are in the [current release record](releases/workspace-facelift-2026-09-12.md).
- **Remaining:** implement the compact daily dashboard below. The dashboard prototype remains separate from the implemented navigation and appearance settings.
...
- **Remaining:** redesign the assistant and page layout, group alerts with durable per-user state, adopt additional package-owned briefing sources, and connect update details to context-aware navigation and editable drafts. Reuse existing Home summaries, briefing preferences and the validated surface bridge.
...
### Marketing engine — remaining phases
