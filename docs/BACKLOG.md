...
- **Remaining:** observed 2026-09-09 — seventeen untracked files (`package-smoke.ts`, `sports-*.ts`) sat in core's `src/app/routes/`, byte-identical to `sports-edge/src-routes/` in the store checkout, with an mtime hours older than the session that found them; a concurrent session removed them by hand. The store compiler stages sources under core's `src/` and clears them in a `finally` ([rebuild-store-routes.mjs](../scripts/security/rebuild-store-routes.mjs) `__oshal_store_parity_`; `oshal-app.js build` has the same shape), so either that cleanup can lose a race or an interrupted build skips it — the exact trigger is not established and reproducing it is the first task. The hazard is Rule 0c: `check-repo-separation.js` only inspects **tracked** paths, so leaked sources pass every gate until someone stages them, and a single `git add -A` would land application code in the kernel.
