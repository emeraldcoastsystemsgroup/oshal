...
- **Delivered (0.3.0, 2026-09-13):** a local electromechanical lab — ngspice in a package-owned engine container, a schematic canvas whose gears, rotors and servo horns turn from the solved angles, DC motors, hobby servos and steppers with their gear trains reflected into the same SPICE solve, twenty-seven parts, a shaft-driver catalog, the gear → CAD Studio hand-off, thirteen route-backed tools for the inline concierge, seven suites registered in the Test Lab, and the operator's external-tool list evaluated under the package's `docs/evaluations/`. See [ADR-154](adr/154-local-electromechanical-lab-circuit-lab.md); the package BACKLOG carries B1 firmware, B2 breadboard, B5 non-rigid mechanics and the evaluation follow-ups B12–B16.
