...
### ADR-139 artifact exchange — the "Send to…" fabric: what is left
- **Done already (2026-09-04→07, all merged, deployed and live-verified):** the spine — registry, owner-bound short-TTL handles, the shared `send-to.js`, the cockpit `artifact=` forward — plus the standard UX tag (three `data-artifact-*` attributes + one include; chip and right-click injected centrally so presentation stays a one-file decision), the shared package-side redeem (`redeemArtifactViaRelay`), and [`scripts/deploy-store-package.sh`](../scripts/deploy-store-package.sh) which copies a package without silently deactivating it. **Destinations:** Email-it, Save-to-OSHAL-Storage, Ingest-to-RAG, Summarize-with-Jarvis (kernel) + Portrait Studio, Career, AI Office, little-monsters Tutor, Print inbox, D&D, Kid Lens, Spaces (apps). **Sources tagged:** files browser, portrait gallery, AI Office files, Video Studio cards, Jarvis deliverable chips, career board resume/cover, career submission screenshots, class materials, lecture audio, venture-plan exports. Verified on the box: pdf offers 8 destinations, json 5, mp4 3, png 4. Design + recipes in [ADR-139](adr/139-artifact-exchange-send-to-registry.md).
- **Remaining — needs a MECHANISM, not a tag:** these sources have no byte-serving URL a handle can point at, so they wait on a decision recorded in the ADR: the task-explorer Files tab (its route answers a JSON preview envelope, `res.json({success,data})`, not bytes), rag-center documents (no per-document byte URL), and every client-generated blob (Switchboard's composed image, camera captures, payroll NACHA files, ocean-lab CAD, workflow-studio JSON). The candidate is a mint-with-bytes handle variant — same owner/TTL semantics, bytes held transiently instead of a locator. **lora** separately has no HTTP image-ingest route at all, so "send a training image" needs that route built first.
- **Remaining — ordinary work:** the little-monsters class-materials DESTINATION (its filing is class-scoped under teacher authority, so it needs a class picker rather than the Tutor's context-free attach); and Stage 4 — a generic "pick an artifact" component over the `provides:` declarations the loader already validates but nothing consumes, plus the NL leg ("Jarvis, send this to X" resolving against `/api/artifacts/actions`).
- **Done when:** the mint-with-bytes decision is recorded in the ADR as yes-with-a-design or no-with-a-reason; a generic picker lists registered `provides:` sources and hands back a minted handle, adopted by at least one surface in place of its bespoke picker; one Jarvis turn dispatches through the registry with the destinations' own confirm gates intact, live-proven; and the class-materials destination either ships (one live dispatch landing in a class) or is struck here with its reason.
