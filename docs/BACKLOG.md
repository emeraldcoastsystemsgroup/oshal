...
### ADR-139 artifact exchange — the everything-rollout (Amendment C waves)
- **Done already:** the standard exists end to end — subscribe (`artifacts:` accepts + the career-idiom `{ref}` import endpoint), the standard UX tag (`data-artifact-source/-type/-name` + one `send-to.js` include; chip + right-click injected centrally, presentation changeable in ONE file), and five live destinations (email, storage, RAG, Jarvis, career) with three integrated surfaces (files browser via the tag, portrait gallery, career). Roster + recipes in [ADR-139 Amendment C](adr/139-artifact-exchange-send-to-registry.md).
- **Wave 1 shipped 2026-09-05/06** (store #123 + jarvis tags, all live-proven): destinations — presentations 2.6.0 (open → outline editor), little-monsters 1.1.0 (open → the TUTOR as pending homework attachment; the class-materials lane moved to wave 2 — it is class-scoped with teacher authority, so it needs a picker), print-ingest 0.2.0 (post → kernel extract-text → its own intake/approval queue; NOTE the package ships `status: inactive` by design and a re-copy resets BOTH the box yaml and the DB toggle — reactivation = box-copy yaml AND swarm_applications row, then restart); sources — presentations My-files rows, Jarvis deliverable chips, Video Studio cards (all via the standard tag). A pdf's menu now offers seven destinations.
- **Wave 2 shipped 2026-09-06** (core #333 + store #125/#126, deployed + verified): the SHARED redeem helper (`redeemArtifactViaRelay`, real-HTTP guarded) and three destinations riding it — dnd 0.20.0 (open → import saves to My Characters), youtube-kids 1.1.0 (post → the same ingest as the raw upload; the app ships `status: inactive` like print-ingest, box-locally activated), spaces 0.6.0 (post → walkthrough video starts a reconstruction; 415 non-video; relay ceiling bounds the clip) — plus sources: little-monsters class materials (standard tag) and the career board's resume/cover anchors (first `data-artifact-ui="context"` use). A pdf's menu now offers EIGHT destinations. The inactive-reset trap fired a THIRD time (the ADR-141 sweep's re-copy cleared print-ingest's box activation): any package re-copy resets box yaml AND the DB toggle both.
- **Remaining:** little-monsters class-materials DESTINATION (needs a class picker over its teacher-authority model); sources not yet tagged: career submission screenshots, task-explorer Files tab, rag-center documents, venture-plan exports, dnd/game-show art, little-monsters lecture audio; switchboard's compose image is blob-gated (wave 3). Wave 3 gates unchanged: lora needs a real HTTP image-ingest route; client-generated blobs need the mint-with-bytes handle variant. Consider a deploy-side guard for the inactive-reset trap (docker-cp helper that preserves box-local `status:`/toggle).
- **Done when:** every wave-1 and wave-2 item either ships (destination = manifest accepts + `{ref}` endpoint proven by one live dispatch landing in the app; source = tagged elements proven by the menu opening on a real item) or is explicitly struck here with a reason; the shared redeem helper exists and at least two packages use it; and the wave-3 gate decisions (lora ingest route yes/no, mint-with-bytes yes/no) are recorded in the ADR.
...
- **Rule 0c:** this ships as a store package in [`oshal-applications`](https://github.com/emeraldcoastsystemsgroup/oshal-applications), sized like `sports-edge`. Core carries only the node half's existing rails and this pointer.
...
- **P0 — the value model and the roster-construction engine (the near-term ask).**
  (a) **Projection to your league's points**: start from ESPN's per-week projection but re-score it under the league's own rules (PPR/half/standard, per-stat points, superflex, TE premium) — a projection is a stat line, not a score. Carry a second independent projection built from the season tape (per-athlete stats, opponent-adjusted defence from `sports-edge`'s Elo/SRS ratings, and its production-weighted injury adjustment) so the engine is never a thin wrapper over one vendor's number, and blend the two at a weight the backtest sets.
  (b) **Replacement level, not rank**: VOR/VORP against a baseline derived from league size × starting slots (a 12-team 2RB/3WR/1FLEX league has a different replacement RB than a 10-team 1FLEX), recomputed as the pool empties.
  (c) **Tiers**, because a draft is a sequence of "which tier breaks next" decisions — detect tier edges by gap size relative to positional variance, never fixed rank cutoffs.
  (d) **VONA (value over next available)**: a pick's true cost is the best player still there at your NEXT pick, so model the intervening picks from ADP with realistic noise and draft for the largest expected gap. This is what separates the engine from a static cheat sheet.
  (e) **"Build from the team out" — marginal value against YOUR roster, not the board**: value a candidate by simulating the rest of the season's weekly OPTIMAL STARTING LINEUP with and without them. A third RB is then worth what it adds when it actually starts, bye-week collisions price themselves, handcuffs price by correlation with the starter, and playoff weeks (15–17) can be weighted above week 3. One mechanism replaces a pile of hand-typed positional rules.
  (f) **Monte Carlo the draft**: for each candidate at the current pick, simulate the remaining rounds N times with opponents drafting from the ADP-noise model, score each finished roster with the lineup optimizer, and recommend on the distribution rather than the mean — upside early, floor late.
  (g) **Auction is the same model with a budget**: max bid is where a player's marginal roster value stops beating the best roster the same dollars buy elsewhere, with inflation recomputed after each sale.
  (h) **The honesty gate, borrowed from `sports-edge` and the Kalshi ledger**: the engine must beat "just follow ADP" on completed prior seasons before anyone calls it an edge — draft from a past season's ADP, then score the resulting roster's actual weekly optimal lineup against rosters drafted straight off ADP. Every real draft's picks are registered before the season and graded after it, and a losing result is reported, not buried.
- **P1 — the site: configuration and commissioner control.** League creation (size, snake/auction/keeper/dynasty, roster slots including superflex, custom per-stat scoring, waiver mode FAAB or rolling, trade deadline and veto policy, playoff weeks and bracket); a draft room (board, queue, timers, autopick, pause/resume, and an offline/paper mode that works with no external league at all); then the in-season half every fantasy site has — matchups, lineup lock, waiver processing, trades with an analyzer built on the same marginal-value model, standings, power rankings, transaction log. Administrative control is its own surface: commissioner edits (rosters, scores, draft order, re-opening a locked lineup), member invite/removal, and an audit trail of every commissioner action. The design question to settle before the schema: package data is `user_sub`-keyed per ADR-036, but a league is a shared object with a member list, so league-scoped isolation has to be explicit rather than inherited.
- **P2 — the live-draft node.** ESPN's private-league and draft-room reads need the operator's `espn_s2` + `SWID` session, so this rides `job-apply`'s rail rather than a connector: a chosen desktop node drives the operator's own logged-in browser, room state returns through the bounded one-use completion callback (`src/features/remote-client/`), and no cookie ever reaches the model, the swarm database, or a log. During a draft the node reports picks as they land; the engine re-runs VONA and the roster simulation and pushes an updated recommendation to the surface. The operator makes every pick — no automated pick into a real draft room without an explicit per-action confirm gate, per the automation opt-in directive. **Blocked by node enrolment** (see "Node enrolment is the blocker for every remote-node capability" above): this swarm sets `REMOTE_CLIENT_REQUIRE_NODE_TOKEN=true` and a freshly installed node is refused, so acceptance runs in a mock draft, never the operator's real league.
- **Done when:** (P0) the store package drafts a full 12-team roster from the real 2026 pool under at least two scoring configurations (PPR and superflex), each pick showing the tier, VOR, VONA and marginal-lineup delta that justified it, and a backtest over a completed season reports the engine's rosters against straight-ADP rosters in total optimal-lineup points across at least 100 simulated drafts — with a loss reported as a loss; (P1) a commissioner creates a league, sets scoring and roster slots, invites a member, runs a draft to completion and edits a roster afterwards entirely from the surface, every commissioner action lands in the audit trail, and a spec proves a second league's admin can neither read nor write the first; (P2) a mock ESPN draft driven in the operator's browser on an enrolled node feeds picks back and the recommendation updates within seconds of each one, with a guard crossing the node boundary that proves the ESPN session cookie never leaves the machine.
...
- **Remaining (store, trading 1.10.0):** the route passes `{ orderType, extendedHours, timeInForce }` to `validateFireAt` and `createDatedOrder`; `ensureEventSchedule` re-creates on a cron/timezone mismatch; the ticket's time input becomes `step="60" min="07:00" max="19:59"` with a client echo of the pre/post rule; the Timed-orders card copy and the README "When" paragraph follow. Until it lands, the deployed 1.9.2 ticket still offers 09:00–09:25 and 16:00–16:55 picks that the kernel now refuses with a 400 naming the rule.
...
- **Remaining:** npm still serves `@oshal/chat@0.2.0` (published 2026-08-15) while core #300 (in-node print service) and #302 (satellite login push) changed the package. **Two of the three blockers are now cleared (2026-09-06, core #339):** the version is bumped to **0.3.0**, and `scripts/npm-parity-check.sh` detects this class of drift (it compares the published version AND the commits after the publish timestamp, because the version numbers MATCHED while the code differed) with the report wired into `oshal-deploy.sh` preflight. The remaining blocker is operator-only: **the npm token expired** — `npm whoami` returns ENEEDAUTH, there is no `.npmrc` and no `NPM_TOKEN` on the box. Renew it, then `bash scripts/npm-publish.sh --publish` (dry-run by default; it refuses an already-published version and refuses to run unauthenticated). Recipe: `docs/runbooks/publishing-the-node-package.md`.
