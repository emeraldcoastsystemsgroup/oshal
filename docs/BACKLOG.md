...
- **Remaining:** the other client-generated exports now have a rail and are simply not tagged yet — Switchboard's composed post image, camera captures, payroll NACHA files, ocean-lab CAD, workflow-studio JSON. Each is the standard tag over a Blob the surface already holds, not new plumbing. **RAG Center documents are NOT in this set** and are not unblocked: a retrieved corpus chunk is not a file, so what a handle would carry there is an open product question, not a missing route.
