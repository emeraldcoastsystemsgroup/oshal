...
- **Open user-testing feedback:** [Profile & Access consistency](backlog/cockpit-workspace-navigation.md#follow-up-profile-and-access-consistency) still needs the corresponding update: remove stale ribbon instructions, align its nested layout/buttons with the selected portal theme and make the existing settings/account destinations clear. Extend and register real profile browser coverage; this is recorded feedback, not an implemented dialog change.
