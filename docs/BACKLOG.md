...
**144 entries open.** 150 closed and recorded in the ledger since it began on 2026-09-20.
...
| Date | Entry | PR |
|---|---|---|
| 2026-09-25 | LoRA gallery image hosting | #828 |
| 2026-09-25 | Data-model explorer: schema drift as an alarm, not a page someone remembers to open | #826 |
| 2026-09-24 | Guide bots that operate their apps | #main |
| 2026-09-24 | Reviewed non-NONE tool provisioning and attestation | #main |
| 2026-09-24 | LinkedIn Content Assistant queue workflow | #main |
| 2026-09-24 | Six host scheduled tasks predate the platform scheduler and should move into it | #main |
| 2026-09-24 | Shared response-renderer completion | #main |
| 2026-09-24 | Kalshi books and execution like the stock side (paper/real × manual/auto) | #main |
| 2026-09-24 | Kalshi: market signals and world news as pre-registered strategies | #main |
| 2026-09-24 | Joke-shorts pump deferred work | #main |
| 2026-09-24 | The watchdog's live books decide cover from the ledger, while the paper book asks the venue (2026-09-21) | #main |
...
- **Remaining:** map production IdP claims/operator allowlists, run restricted/operator/admin route probes, and enable `OSHAL_RBAC_ENFORCE`. The JSON/CSV endpoints now emit a hash-chained artifact with optional operator-key anchoring; retain and independently verify exports using [the integrity runbook](security/audit-export-integrity.md).
...
- **Remaining:** add Slack behind explicit confirmation, then Teams and Twilio link delivery when their credentials exist; preserve artifact ownership and expiry. Rendering, import, and owner-scoped package foundations are guarded, but no live recipient-facing adapter proof exists yet.
...
### Calling Assistant account configuration and live handoff — operator action
- **Status:** OPEN — needs live proof

**Open, requires operator setup and a real carrier receipt.** The optional Calling Assistant package
provides the calling tool, bot, workflow, owner configuration and report. Open its Calling screen,
select an existing Twilio connection and voice-capable sender, set the handoff phone, public HTTPS
origin, permitted destinations, speech provider, disclosure and limits. Calling remains disabled
until the owner opts in; connecting Twilio alone is insufficient.

Use the audio upload check, then run a controlled destination and answer the handoff phone. Retain
the report showing actual provider call status, keypad/spoken responses and completed handoff.
Local generated-WAV and fake-carrier tests do not close this operator item. See the store package's
`calling-assistant/README.md` for the setup procedure and explicit execution/configuration scope.
