...
- **Done when:** met for the purge — bounded and fail-loud (helper + spec timeout case); clears a real `node_modules` export on this box in minutes (measured on the real leftover `ci-src` from the 2026-09-14 00:15 run: one `purge: OK ... (70s)` line for `%LOCALAPPDATA%\oshal\ci-src`, rc=0, path gone — the two write-ups of that measurement recorded different file counts for the tree and the purge consumed it, so no count is claimed for it); a run that inherits a leftover export reaches its gates and writes an outcome line (a purge FAIL is a `head-src` gate FAIL, which today already continues to the remaining gates and the outcome line).
