...
- **Skin and optional navigation deployed and verified:** Workspace uses the existing CSS theme system; a separate Navigation setting adds top workspaces while retaining the sidebar. Sidebar remains the default. Existing theme choices and packaged screens remain available. Current profile synthesis and application permissions determine destinations; Career prefers its admitted group or falls back to its admitted application. The focused appearance suite passed 114 checks across five files, including embedded chat. Navigation passed 50 profile/authorization cases (28 new navigation HTTP cases), plus 18 Chromium cases, four model cases and one Lab registration assertion. Another 29 existing catalog regressions passed separately. Slow discovery remains usable within a bounded 30-second window; expired requests show Retry without retaining application links.
- **Remaining:** implement the compact daily dashboard below. The dashboard prototype is still separate from the deployed navigation and color settings; focused native acceptance and preservation checks are recorded in the [release record](releases/parallel-backlog-2026-09-11.md).
