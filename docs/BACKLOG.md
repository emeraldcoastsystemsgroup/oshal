...
### One bot-invocation chokepoint — route /api/send-message through executeBotOrInline (+ the missing ADR)
- **Remaining:** `handleSendMessage` (and its `/api/tasks/:taskId/messages` alias) still calls `ctx.orchestrator.processMessage` directly, bypassing the sanctioned chokepoint — so the cockpit chat panel misses the ADR-104 budget gate, the ADR-090 skill-profile carrier, the credential-carrier refusals, and (structurally) bot-node dispatch; only entitlement + the ADR-127 brain ladder were back-ported (SECURITY-HARDENING §1.4b). Extend `executeBotOrInline` with optional `ticketContext`/`interactionMode` passthrough so chat-ticket bookkeeping survives, add node-turn thread persistence (user + assistant `messageStore.save`), then route the handler through it. Write the ADR the guides assume but no ADR owns ("bot invocation — one chokepoint"), enumerating as migration debt the ~11 store packages calling `processMessage` directly (dnd, spotify, travel, purchasing, movies, aero-lab, camera, drone, sat-ops, game-show, bake-off engines) and `swarm-control.js`'s browser→bot direct POST.
- **Done when:** a cockpit chat turn to any bot passes the budget gate and credential refusals (guard proves a HARD-cap breach blocks it), a node bot's direct chat executes at its node with thread history intact, the ADR is in the index, and the Tier-C call-site list is tracked with owners.

### Career-hunter bot node (ADR-093 Tier 2) — the mounted-CLI brain for packaged chat
- **Remaining:** the operator wants packaged-bot chat on the bind-mounted CLI logins; inline-on-controller can never spawn them (SEC-05), so cb…0001 needs a node. Phase-1b design done, deliberately not landed: (a) `manifestBotDefinition` needs an optional `container:`/`port:` passthrough on `SwarmAppBotDeclaration` (defaults unchanged); (b) executeBotOrInline's remote branch needs ADR-127 hosted threading for non-carve callers (skip carve callers via `cliBrainAvailable` — the node's registry harness serves them) and a decision on node-optional dispatch (registry says `container: career-bot` but the node is opted off ⇒ dispatch must fall back inline, not ECONNREFUSED the chat); (c) the Tier-2 fragment MECHANICS have never been exercised — compose `extends` is blocked by `depends_on`, YAML anchors don't cross `-f` files, and hand-copying the ~250-line `x-bot-env` contract into a package fragment is a drift machine. Candidate alternatives: a compose `profiles:`-gated first-party service, or a generated fragment rendered from the core template.
- **Done when:** with the node enabled, the operator's career chat and Resume Studio guide answer with `provider: claude-code` under cb…0001 at the career node (carve line in its log); with the node disabled the same turns degrade to the hosted ladder, not an error; guests thread `byoLlmConnection` at the node; and the chosen fragment mechanism is documented in ADR-093 or a successor.
