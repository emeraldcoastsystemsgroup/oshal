...
- **Remaining:** implement the compact daily dashboard below. The September 13 follow-up promotes Finance to a named area and puts miscellaneous destinations in the OSHAL menu, without an `Other` dashboard bucket. See [Finance and the OSHAL menu](backlog/cockpit-workspace-navigation.md#requested-follow-up-finance-and-the-oshal-menu). These remain backlogged changes; the existing navigation and appearance settings are separate delivered work.
