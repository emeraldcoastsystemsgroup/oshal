...
**253 entries open.** 40 closed and recorded in the ledger since it began on 2026-09-20.

| Status | Count | Meaning |
|---|---|---|
| IN PROGRESS | **0** | being worked in the current session |
| OPEN — actionable | **93** | no decision, no live box needed; can be closed by an agent |
...
| OPEN — untriaged | **40** | filed after the 2026-09-15 triage; has no verdict yet |
...
| Date | Entry | PR |
|---|---|---|
| 2026-09-24 | Three CLI tools reach a shell with model-supplied arguments and require no approval (2026-09-21) | #817 |
| 2026-09-24 | Trading — cash settlement: three narrower gaps (ADR-134 D8) | #780 |
| 2026-09-24 | print-to-rag has no user guide and no page on the site | #n/a |
| 2026-09-24 | Ambient Recall — attributed ingest fixture for the Test Lab | #728 |
| 2026-09-24 | `swarm-cli` zsh completion | #521 |
| 2026-09-24 | Disposable PostgreSQL fixture leaks its data volume on every dispose | #699 |
| 2026-09-24 | The publish gate's attribution wall can be disabled by author, and no case would notice | #n/a |
| 2026-09-24 | A protected dispatch refuses on a controller with no delegation signing material (2026-09-15) | #n/a |
| 2026-09-24 | Flow UI-automation video provider | #n/a |
| 2026-09-24 | Echo pipeline snapshots into the DevOps monitoring / self-healing funnel | #810 |
| 2026-09-24 | Data-model explorer: one implementation behind both the docs and the surface | #727 |
| 2026-09-24 | No gate typechecks a test file, so "typecheck clean" says nothing about one | #808 |
| 2026-09-24 | Concierge and tool coverage is not a contract (P8) - and its first done-when was wrong | #252 (store), 799 |
| 2026-09-24 | Automated post-deploy product proof must remain binary under delegation signing (2026-09-16) | #798, 804 |
...
- **Status:** OPEN — untriaged
- **Status:** OPEN — needs live proof · Merged in PR #626 (177745ba), guarded by tests/unit/bot-node-database-pool-recovery.spec.ts. Awaiting live proof: built-image probe against real deployment container.
...
- **Status:** OPEN — actionable
- **Status:** OPEN — needs live proof · Advisory lock bootstrap race closed and guarded by tests/unit/trading-schema-bootstrap-race.spec.ts; remaining test timeouts and Postgres/Redis environment constraints tracked under disposable-PostgreSQL entry.
...
- **Status:** OPEN — untriaged
- **Status:** OPEN — needs live proof · Store package little-monsters 1.3.5 (commit ef58e99) resolves migrations/019-education-platform.sql from package directory first with fallback to cwd. Awaiting live proof: fresh api boot logging no missing-migration error.
...
- **Status:** OPEN — actionable
- **Status:** OPEN — needs live proof · Merged in PR #723 (57e87692), rate limit retry and step isolation guarded by tests/unit/trading-market-data-rate-limit.spec.ts and tests/unit/trading-assess-step-failure.spec.ts. Awaiting live proof: two consecutive nightly assessments recording per-algo detail without rate limit.
...
- **Implemented 2026-09-24 (core PR #815, store PR #255):** `google-calendar` is contracted and pinned as a kernel skill in `src/app/composition/kernel-skills.ts`, `src/shared/kernel-skills/registry.ts`, and guarded by `tests/unit/kernel-skills.spec.ts`. Verified present in server compile list (`tsc -p tsconfig.server.json --listFilesOnly`). The `calendar` store package declares `uses: [google-calendar]` in its manifest and imports via `@/features/google-calendar` and `@/features/person-model` barrels with zero `@/features/*/services/*` deep imports. Compatibility floor (`>= 2.1.0-beta.1`) documented in `docs/apps/kernel-skills.md`. Awaiting post-deploy verification of package activation and sync route response on the live box.
...
- **Status:** OPEN — actionable
- **Status:** OPEN — needs live proof · Code and guards completed on 2026-09-17 in src/features/person-model/services/related-relevance.ts and tests/unit/person-model-related-relevance.spec.ts. Awaiting live proof: docker exec oshal-local-api node /tmp/pm-live.js prints LIVE PROOF OK.
