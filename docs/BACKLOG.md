...
- **Part of P0 already ships in sports-edge (verified on the box 2026-09-09, 0.4.1 active) — do not rebuild it.** `/api/sports-edge/fantasy/` already serves status, league link/unlink, **the optimal lineup and start/sit calls written to the ledger before kickoff**, the graded record, and a cached shared projection refresh, over `applyScoring` (from the league's own `scoringItems`), `optimiseLineup`, `startSitCalls` and `distilProjections` — with the credential rule already implemented (resolved per request, used outbound, never logged, stored or returned). The GAP is: rest-of-season `SV`/`MV` (the shipped optimiser is one week), the `P(win)` objective and the spread it needs, waivers with a bid, the two-sided trade finder plus the other nine rosters it reads, streaming, and matchup awareness (`mMatchup` is unread, so nothing knows who you play). **And none of the shipped half has ever run against a real league** — there is no `espn-fantasy` row in `oshal_connections`, so it has only seen fixtures.
...
### ADR-145 Home view: merged but NOT yet verified on the deployed stack (2026-09-09)
- **Remaining:** the Home landing page (core PR #380, `73a9bfe0`) is on `main` and green in unit tests, but the deploy that would have put it on the box FAILED and auto-rolled back: `scripts/oshal-deploy.sh` built the image (rev 73a9bfe0, 06:23:58Z) and then hit `api healthy but auto-load never completed`, so it restored `cec3ebd38a83` (rev be031863) and re-tagged `oshal-bot:latest` to it. The built image is now UNTAGGED, so a retry needs a full rebuild (~18 min), not `--skip-build`. Evidence the ADR-145 code is not the cause: `readManifest` over all 61 real manifests (swarm-apps/ + the store checkout) passes 61/61 with 0 failures mentioning `summary`, and the validator returns early when the key is absent — no manifest declares it yet. Evidence for the real cause: the Docker daemon returned HTTP 500 on `/containers/.../json` and `/images/.../json` at 01:40 local, INSIDE the deploy's 01:25->01:56 gate window; that gate polls `docker inspect` + `docker logs` for the auto-load marker and burns its iterations when the daemon errors.
- **Done when:** `bash scripts/oshal-deploy.sh` (UNPIPED — piping masks its exit code, which is how this run first looked like exit 0) completes with exit 0 on a healthy daemon, `GET /api/swarm/apps/home-plan` returns cards for a signed-in user, and the cockpit Home view renders one card per active group/app with a broken probe showing "can't check" rather than a green state.
