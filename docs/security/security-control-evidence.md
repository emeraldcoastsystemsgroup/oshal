...
| Session-less CLI-token (PAT) bootstrap mint is operator-only and time-boxed: a service-secret request asserting a non-operator sub cannot mint (`403`), and operator mints expire per `OSHAL_CLI_TOKEN_BOOTSTRAP_TTL_DAYS` (default 30d). Closes the pre-fix cross-user PAT takeover. | `isOperatorIdentity` + `bootstrapTtlMs` on `POST /api/cli-tokens` - `src/app/routes/cli-token-routes.ts` | `tests/unit/cli-token-auth.spec.ts` | Enforced; Proven (exploit re-tested live after PR #83 deploy → `403 operator_required`) |
| Worker bots run a least-privilege DB role: bot-node containers connect as `oshal_bot` (NOSUPERUSER, NOBYPASSRLS, DML-only), not the operator superuser DSN, so a compromised bot cannot bypass RLS. | `BOT_DATABASE_URL` → `oshal_bot`; migration `099-bot-db-role.sql` | `tests/unit/bot-db-least-privilege.spec.ts` | Enforced; live-verified 2026-08-01 (bots report `oshal_bot`) |
