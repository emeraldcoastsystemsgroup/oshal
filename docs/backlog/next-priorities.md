...
| 7 | Dispatched specialists receive authorized application data | A package-owned deterministic read supplies normalized data to the existing accounted path; known-answer, ownership, refusal and credential-exclusion tests pass. | Source verified; final checkpoint pending |
...
| 9 | Users control Jarvis briefings | Registered sources expose per-user enable/frequency/channel preferences; announcement cadence honors them and disabling stops delivery; untouched eligible defaults remain compatible. | Source verified; final checkpoint pending |
| 10 | Administrators can inspect authorization change history | Scoped, paginated audit reads and a working Access screen show actor/change/policy revision; unauthorized and cross-scope reads refuse. | Source verified; final checkpoint pending |
