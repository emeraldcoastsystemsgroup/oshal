...
- [ ] Run Node-harness Playwright recipes in the disposable sandbox so the
  ...
  verified capabilities; the catalog re-seals on verification and refuses
  recipes without the `node:test` harness; the run service admits the kind;
  the api probes once per boot (`OSHAL_TEST_LAB_RUNNER_PROBE=off` skips it).
  Pure admission, harness and re-seal proofs pass. The real-boundary proof has
  NOT run — see "Test Lab browser runner: the real-boundary proof has not run"
  in [the project backlog](../BACKLOG.md) for why, the command, and the
