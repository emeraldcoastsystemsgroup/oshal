...
- [provider-fallback-review-followups.md](./provider-fallback-review-followups.md) — **CLOSED 2026-09-18**: the record of the five-lens adversarial review of PR #645 that returned **block** with 28 findings (3 blocking, 15 should-fix, 10 nit), all now fixed. Kept because of HOW it nearly went unfixed: the review's output file was empty after a network outage, was treated as lost, and a self-check by the author was substituted — so #645 merged with the operator's actual requirement broken (a rung that was not a native runtime name was silently dropped at the node) and main's suite red. The findings themselves are a catalogue worth re-reading: seven durable places asserting a capability that did not exist, four guards that passed on the bug they guarded, a status denylist that would return an unknown vendor status as the model's answer, and three separate fixes that reached one of the two places that needed them.
- [boot-migration-silent-skip.md](./boot-migration-silent-skip.md) — **observed live 2026-09-19**: an api whose boot migration pass failed on its first pool acquire still reached HEALTHY, answered /health 200, completed swarm-app auto-load and served traffic — with migration 148 never applied and the `fallback_order` column absent. The chain is void-ed with a catch that deliberately releases the gated consumers to self-heal; the consumers do, the MIGRATIONS do not. So a deploy can report success over a schema older than its own code. Done-when: the failure reaches the health surface the deploy gates on, the pass retries, and the log names the outstanding migration.
