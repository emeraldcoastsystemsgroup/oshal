...
- [provider-fallback-review-followups.md](./provider-fallback-review-followups.md) — the 21 findings from the five-lens adversarial review of PR #645 that are **not** yet fixed (the 3 blocking ones and 4 others were). Each was verified by executing the code. Highlights: `OSHAL_PROVIDER_AUTO_FAILOVER` is inert in both arms while `.env.example` calls it the kill switch (F5); the api-side `environment` rung has zero callers, so the documented ladder is three rungs, not four (F6); "usable as a fallback rung" is asserted for `gemini-cli`/`antigravity-cli` in five durable places and is false for both (F7); the two Antigravity guards are source-text greps the change-log prose alone satisfies (F9); the cockpit destroys the stored chain on every save because it cannot express NULL (F12). Also records how a re-review that returned **block** was treated as lost and the PR merged without it.
