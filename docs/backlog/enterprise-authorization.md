...
| AUTH-05 | P0 enforcement lane | Shared evaluator at package HTTP, tools, artifacts and bot/queue boundaries; user-scoped Jarvis YAML/catalog/context; current-rights checks on execution | Raw API and direct/queued/MCP alternatives cannot bypass denial; an enabled tool is hidden/refused for a restricted user; revoked queued work never executes. Tokens and service allowlists only narrow grants; missing user delegation never becomes system. |
...
| AUTH-07 | P1 core installation + UI | Atomic catalog lifecycle, transitive privilege-expansion review, existing Users/Applications access management and explain view; coordinate TLAB-02/05 metadata/history | Install/update/reload/disable/uninstall reconcile policy and Lab entries; unchanged roles cannot widen through referenced field/scope/operation changes; stale/removed/source-replaced grants cannot revive; policy revisions and retained stale results are supported before claimed. Ordinary user cannot edit grants; scoped app access manager cannot alter another app/swarm role. Concurrent policy edits have clear feedback. |
...
| `authorization-policy-matrix` | Unit + isolated DB | Direct/group grants, explicit deny, legacy tier ceiling, effect/tier consistency, grant expiry, namespace isolation; viewer cannot mutate through any transport; own-details + team-summary never returns team-details. |
...
| `authorization-admin-browser` | Browser + fixture identities | Existing Users/Applications UI grants/restricts only within admin ceiling, renders provenance and effective deny, prevents self-escalation and never exposes business data through explain. |
